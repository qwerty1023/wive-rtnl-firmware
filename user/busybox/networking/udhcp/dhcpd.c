		...
		unsigned arpping_ms)
{
	...
	if (!static_lease_nip) {
		...
		/* Or: if client has requested an IP */
		 ...
		 && (  !(lease = find_lease_by_nip(req_nip)) /* is not already taken */
			...
			addr.s_addr = req_nip;
			bb_info_msg("Host requested %s", inet_ntoa(addr));
	...
	bb_info_msg("Sending OFFER of %s to %02x:%02x:%02x:%02x:%02x:%02x", inet_ntoa(addr), packet.chaddr[0], packet.chaddr[1], packet.chaddr[2], packet.chaddr[3], packet.chaddr[4], packet.chaddr[5]);
...
static NOINLINE void send_NAK(struct dhcp_packet *oldpacket)
{
	...
	bb_info_msg("Sending NAK to %02x:%02x:%02x:%02x:%02x:%02x", packet.chaddr[0], packet.chaddr[1], packet.chaddr[2], packet.chaddr[3], packet.chaddr[4], packet.chaddr[5]);
...
static NOINLINE void send_ACK(struct dhcp_packet *oldpacket, uint32_t yiaddr)
{
	...
	addr.s_addr = yiaddr;
	bb_info_msg("Sending ACK to %02x:%02x:%02x:%02x:%02x:%02x for %s", packet.chaddr[0], packet.chaddr[1], packet.chaddr[2], packet.chaddr[3], packet.chaddr[4], packet.chaddr[5], inet_ntoa(addr));
	send_packet(&packet, /*force_bcast:*/ 0);
...
int udhcpd_main(int argc UNUSED_PARAM, char **argv)
{
 ...
 continue_with_autotime:
	...
	while (1) { /* loop until universe collapses */
		...
		if (static_lease_nip) {
			struct in_addr addr;
			addr.s_addr = static_lease_nip;
			bb_info_msg("Found static lease for %02x:%02x:%02x:%02x:%02x:%02x: %s", packet.chaddr[0], packet.chaddr[1], packet.chaddr[2], packet.chaddr[3], packet.chaddr[4], packet.chaddr[5], inet_ntoa(addr));
		...
		case DHCPDISCOVER:
			bb_info_msg("Received DISCOVER from %02x:%02x:%02x:%02x:%02x:%02x", packet.chaddr[0], packet.chaddr[1], packet.chaddr[2], packet.chaddr[3], packet.chaddr[4], packet.chaddr[5]);
		...
		case DHCPREQUEST:
			if (requested_ip_opt) {
				struct in_addr addr;
				addr.s_addr = requested_nip;
				bb_info_msg("Received REQUEST from %02x:%02x:%02x:%02x:%02x:%02x with %s", packet.chaddr[0], packet.chaddr[1], packet.chaddr[2], packet.chaddr[3], packet.chaddr[4], packet.chaddr[5], inet_ntoa(addr));
			}
			else {
				bb_info_msg("Received REQUEST from %02x:%02x:%02x:%02x:%02x:%02x", packet.chaddr[0], packet.chaddr[1], packet.chaddr[2], packet.chaddr[3], packet.chaddr[4], packet.chaddr[5]);
            }
/* RFC 2131:
...
*/
			if (!requested_ip_opt) {
				...
				if (requested_nip == 0) {
					bb_info_msg("No requested IP set, ignoring.");
					break;
				}
			}
