...
function showHint(key)
{
	...
	else if (form.vpn_enabled.checked)
	{
		...
		{
			if ((form.vpn_type.value == '1') || (form.vpn_type.value == '2')) // PPTP client, L2TP client
		...
		else if (key=='vpn_dgw')
		{
			...
			text += 'Enable pure PPPoE mode without DHCP.';
		else if (key=='vpn_cpu_limit')
			text += 'This option prevents full CPU load by ppp kernel thread. Minimum value is 1000, maximum value is 6000. Zero value disables this option. Recommended value is 3200 with MPPE On or 4500 with MPPE Off.';
...
{
	enableElements( [
		form.vpn_server, form.vpn_pppoe_ac, form.vpn_auth_type,
...
function selectType(form)
{
	...
	displayElement([ 'vpn_auth_type_row', 'vpn_user_row', 'vpn_mtu_row', 'vpn_dgw_row', vpn_mppe_row, table_vpn_params], !kabinet_on);
	displayElement([ 'vpn_pure_pppoe_cell', 'vpn_pppoe_ac_row', 'vpn_pppoe_service_row', 'vpn_pppoe_row' ], pppoe_on);
	displayElement([ 'vpn_server_row', 'vpn_test_reachable' ], pptp_on || l2tp_on);
	displayElement('vpn_lanauth_lvl_row', kabinet_on);

	var vpn_server_text = '';
	if ((form.vpn_type.value == '1') || (form.vpn_type.value == '2')) // PPTP client, L2TP client
		vpn_server_text = 'Host, <acronym title="Internet Protocol">IP</acronym> or <acronym title="Domain Name System">DNS</acronym> name:';
	else if ((form.vpn_type.value == '4') || (form.vpn_type.value == '5'))
		vpn_server_text = '<acronym title="Access Point Name">APN</acronym> name:'; // GSM/CDMA
	
	vpn_server_col.innerHTML = vpn_server_text;
}
...
{
	var reset_timer = function() { self.setTimeout('showVPNStatus();', 2500); };
...
function formCheck(form)
{
	...
	if ((vpn_cpu_limit != 0) && ((vpn_cpu_limit < 1000) || (vpn_cpu_limit > 6000)))
...
<table class="form">
	...
	<tr>
		<td class="head" onMouseOver="showHint('vpn_enabled')" onMouseOut="hideHint('vpn_enabled')" >
			...
			Enable <acronym title="Virtual Private Network">VPN</acronym>
		</td>
	...
	<tr onMouseOver="showHint('vpn_type')" onMouseOut="hideHint('vpn_type')" >
		<td class="head">
			<acronym title="Virtual Private Network">VPN</acronym> mode:
		</td>
		<td>
	...
	<tr id="vpn_pppoe_row" style="display: none;" onMouseOver="showHint('vpn_pppoe_iface')" onMouseOut="hideHint('vpn_pppoe_iface')">
		<td class="head">PPPoE interface:</td>
		<td>
	...
	<tr id="vpn_pppoe_ac_row" onMouseOver="showHint('vpn_pppoe_ac')" onMouseOut="hideHint('vpn_pppoe_ac')">
		<td class="head"><acronym title="Access Concentrator">AC</acronym> name:</td>
		<td><input name="vpn_pppoe_ac" class="mid" value="<% getCfgGeneral(1, "vpnAc"); %>" disabled="disabled" type="text"></td>
	</tr>
	<tr id="vpn_pppoe_service_row" onMouseOver="showHint('vpn_pppoe_service')" onMouseOut="hideHint('vpn_pppoe_service')">
		<td class="head">Service name:</td>
	...
	<tr id="vpn_server_row" onMouseOver="showHint('vpn_server')" onMouseOut="hideHint('vpn_server')">
		<td class="head" id="vpn_server_col">Host, <acronym title="Internet Protocol">IP</acronym> or <acronym title="Domain Name System">DNS</acronym> name:</td>
	...
	<tr id="vpn_auth_type_row" onMouseOver="showHint('vpn_auth_type')" onMouseOut="hideHint('vpn_auth_type')" >
		<td class="head">Authentication method:</td>
		<td>
	...
	<tr id="vpn_lanauth_lvl_row" onMouseOver="showHint('vpn_lanauth_access')" onMouseOut="hideHint('vpn_lanauth_access')">
		<td class="head">KABINET access level:</td>
		<td>
	...
	<tr id="vpn_mtu_row" onMouseOver="showHint('vpn_mtu')" onMouseOut="hideHint('vpn_mtu')" >
		<td class="head"><acronym title="Maximum Transfer Unit">MTU</acronym>/<acronym title="Maximum Recieve Unit">MRU:</acronym></td>
		<td>
	...
	<tr id="vpn_dgw_row" onMouseOver="showHint('vpn_dgw')" onMouseOut="hideHint('vpn_dgw')" >
		...
		<td>
			<select disabled="disabled" name="vpn_dgw" class="mid" >
...
<table class="form" id="table_vpn_params04">
	...
	<tr id="vpn_mppe_row">
		<td width="50%" onMouseOver="showHint('vpn_mppe')" onMouseOut="hideHint('vpn_mppe')" >
			...
			<b>Require <acronym title="Microsoft Point-to-Point Encryption">MPPE</acronym></b>
		</td>
...
<table class="buttons">
	<tr>
		<td>
			...
			<input class="normal" value="Apply" name="save" type="submit" onclick="return submitClick(this.form);" >&nbsp;&nbsp;
