...
bool init_names(void)
{
	...
	if (global_myname() == NULL || *global_myname() == '\0') {
		if (!set_global_myname(myhostname())) {
			DEBUG(1, ( "init_structs: malloc fail.\n" ) );
			return False;
		}
	}
		       ...
		       bool parent_longlived)
{
	...
	if (msg_ctx) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG(1,("messaging_reinit() failed: %s\n",
				 nt_errstr(status)));
			return false;
		}
	}
...
void *Realloc(void *p, size_t size, bool free_old_on_error)
{
	...
	if (!ret) {
		...
		DEBUG(1,("Memory allocation error: failed to expand to %d bytes\n",(int)size));
	}
...
int interpret_protocol(const char *str,int def)
{
	...
	DEBUG(1,("Unrecognised protocol level %s\n",str));
...
void smb_panic(const char *const why)
{
...
#ifdef DEVELOPER
	{
		...
		if (global_clobber_region_function) {
			DEBUG(1,("smb_panic: clobber_region() last called from [%s(%u)]\n",
	...
	DEBUG(1,("PANIC (pid %llu): %s\n",
		    (unsigned long long)sys_getpid(), why));
	log_stack_trace();

	cmd = lp_panic_action();
	if (cmd && *cmd) {
		DEBUG(1, ("smb_panic(): calling panic action [%s]\n", cmd));
		result = system(cmd);

		if (result == -1)
			DEBUG(1, ("smb_panic(): fork failed in panic action: %s\n",
					  strerror(errno)));
		else
			DEBUG(1, ("smb_panic(): action returned status %d\n",
					  WEXITSTATUS(result)));
	}
...
libunwind_failed:
	DEBUG(1, ("unable to produce a stack trace with libunwind\n"));
	...
	DEBUG(1, ("BACKTRACE: %lu stack frames:\n", 
...
#define NAMESIZE 32 /* Arbitrary */
	...
	DEBUG(1, ("BACKTRACE: %d stack frames:\n", levels));
	for (i = 0; i < levels; i++) {
...
#else
	DEBUG(1, ("unable to produce a stack trace on this platform\n"));
#endif
}
...
void set_namearray(name_compare_entry **ppname_array, const char *namelist)
{
	...
	*/
	...
	if(( (*ppname_array) = SMB_MALLOC_ARRAY(name_compare_entry, num_entries + 1)) == NULL) {
		DEBUG(1,("set_namearray: malloc fail\n"));
		return;
	}
	...
	while(*nameptr) {
		...
		if(((*ppname_array)[i].name = SMB_STRDUP(nameptr)) == NULL) {
			DEBUG(1,("set_namearray: malloc fail (1)\n"));
			return;
		}
