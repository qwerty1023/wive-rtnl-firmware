					  ...
					  struct cli_state **cli)
{
	...
	status = cli_rpc_pipe_open_noauth(*cli, &ndr_table_lsarpc.syntax_id,
					  &pipe_hnd);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1,("Error connecting to LSA pipe. Error was %s\n",
			nt_errstr(status)));
		goto done;
	}
					   ...
					   struct cli_state *cli)
{
	...
	status = cli_rpc_pipe_open_noauth(cli, &ndr_table_samr.syntax_id,
					  &pipe_hnd);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1,("Error connecting to SAM pipe. Error was %s\n",
			nt_errstr(status)));
		goto done;
	}
	...
	if (name_types.ids[0] != SID_NAME_USER) {
		DEBUG(1,("%s is not a user account (type=%d)\n",
			...
			const char *dc_name)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		DEBUG(1,("libnet_join_ok: failed to get schannel session "
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1,("libnet_join_ok: failed to open schannel session "
