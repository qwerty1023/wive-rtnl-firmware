...
NTSTATUS smb_register_passdb(int version, const char *name, pdb_init_function init) 
{
	...
	if(version != PASSDB_INTERFACE_VERSION) {
		DEBUG(1,("Can't register passdb backend!\n"
	...
	if (pdb_find_backend_entry(name)) {
		DEBUG(1,("There already is a passdb backend registered with the name %s!\n", name));
...
NTSTATUS make_pdb_method_name(struct pdb_methods **methods, const char *selected)
{
	...
	if (!entry) { 
		...
		if(NT_STATUS_IS_OK(smb_probe_module("pdb", module_name)) && !(entry = pdb_find_backend_entry(module_name))) {
			DEBUG(1,("Plugin is available, but doesn't register passdb backend %s\n", module_name));
	...
	if(!entry) { 
		DEBUG(1,("No builtin nor plugin backend for %s found\n", module_name));
		SAFE_FREE(module_name);
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if ( !NT_STATUS_IS_OK( nt_status = entry->init(methods, module_location) ) ) {
		DEBUG(1,("pdb backend %s did not correctly init (error was %s)\n", 
...
bool guest_user_info( struct samu *user )
{
	...
	if ( !(pwd = getpwnam_alloc(talloc_autofree_context(), guestname ) ) ) {
		DEBUG(1,("guest_user_info: Unable to locate guest account [%s]!\n", 
			guestname));
		return False;
	}
					...
					uint32 acb_info, uint32 *rid)
{
	...
	if (!sid_peek_check_rid(get_global_sam_sid(),
		...
		DEBUG(1, ("Could not get RID of fresh user\n"));
...
static int smb_delete_user(const char *unix_user)
{
	...
	if ( strequal( unix_user, "root" ) ) {
		DEBUG(1,("smb_delete_user: Refusing to delete local system root account!\n"));
		return -1;
	}
...
bool pdb_new_rid(uint32 *rid)
{
	...
	if (pdb_rid_algorithm()) {
		DEBUG(1, ("Trying to allocate a RID when algorithmic RIDs "
			  "are active\n"));
		return False;
	}

	if (algorithmic_rid_base() != BASE_RID) {
		DEBUG(1, ("'algorithmic rid base' is set but a passdb backend "
	...
	if ( (ctx = talloc_init("pdb_new_rid")) == NULL ) {
		DEBUG(1,("pdb_new_rid: Talloc initialization failure\n"));
		return False;
	}
	...
	if ( allocated_rid == 0 ) {
		DEBUG(1,("pdb_new_rid: Failed to find unused RID\n"));
		return False;
	}
				   ...
				   DOM_SID *sid)
{
	...
	if ( !(sampw = samu_new( NULL )) ) {
		DEBUG(1,("pdb_default_uid_to_rid: samu_new() failed!\n"));
		return False;
	}
