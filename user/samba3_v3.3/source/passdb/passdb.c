...
struct samu *samu_new( TALLOC_CTX *ctx )
{
	...
	if ( !(user = TALLOC_ZERO_P( ctx, struct samu )) ) {
		DEBUG(1,("samuser_new: Talloc failed!\n"));
		return NULL;
	}
...
int algorithmic_rid_base(void)
{
	...
	if (rid_offset < BASE_RID) {  
		...
		DEBUG(1, ("'algorithmic rid base' must be equal to or above %ld\n", BASE_RID));
		rid_offset = BASE_RID;
	}
	if (rid_offset & 1) {
		DEBUG(1, ("algorithmic rid base must be even\n"));
		rid_offset += 1;
	}
	return rid_offset;
}
	...
	if ((flags & LOOKUP_NAME_GROUP) == 0) {
		...
		if (ret) {
			if (!sid_check_is_in_our_domain(&user_sid)) {
				DEBUG(1, ("User %s with invalid SID %s in passdb\n",
...
static bool init_samu_from_buffer_v0(struct samu *sampass, uint8 *buf, uint32 buflen)
{
	...
	if(sampass == NULL || buf == NULL) {
		DEBUG(1, ("init_samu_from_buffer_v0: NULL parameters found!\n"));
		return False;
	}
...
static bool init_samu_from_buffer_v1(struct samu *sampass, uint8 *buf, uint32 buflen)
{
	...
	if(sampass == NULL || buf == NULL) {
		DEBUG(1, ("init_samu_from_buffer_v1: NULL parameters found!\n"));
		return False;
	}
...
static bool init_samu_from_buffer_v2(struct samu *sampass, uint8 *buf, uint32 buflen)
{
	...
	if(sampass == NULL || buf == NULL) {
		DEBUG(1, ("init_samu_from_buffer_v2: NULL parameters found!\n"));
		return False;
	}
...
static bool init_samu_from_buffer_v3(struct samu *sampass, uint8 *buf, uint32 buflen)
{
	...
	if(sampass == NULL || buf == NULL) {
		DEBUG(1, ("init_samu_from_buffer_v3: NULL parameters found!\n"));
		return False;
	}
...
/* SAMU_BUFFER_FORMAT_V3       "dddddddBBBBBBBBBBBBddBBBdwdBwwd" */
	...
	if ( (*buf=(uint8*)SMB_MALLOC(len)) == NULL) {
		DEBUG(1,("init_buffer_from_samu_v3: Unable to malloc() memory for buffer!\n"));
		return (-1);
	}
	...
	if (buflen != len) {
		DEBUG(1, ("init_buffer_from_samu_v3: somthing odd is going on here: bufflen (%lu) != len (%lu) in tdb_pack operations!\n", 
...
bool pdb_update_bad_password_count(struct samu *sampass, bool *updated)
{
	...
	if (!res) {
		DEBUG(1, ("pdb_update_bad_password_count: pdb_get_account_policy failed.\n"));
		return False;
	}
...
bool pdb_update_autolock_flag(struct samu *sampass, bool *updated)
{
	...
	if (!res) {
		DEBUG(1, ("pdb_update_autolock_flag: pdb_get_account_policy failed.\n"));
		return False;
	}
...
bool pdb_increment_bad_password_count(struct samu *sampass)
{
	...
	unbecome_root();
	if ( !ret ) {
		DEBUG(1, ("pdb_increment_bad_password_count: pdb_get_account_policy failed.\n"));
		return False;
	}
			...
			const char **account_name, uint32 *channel)
{
	...
	if (is_dc_trusted_domain_situation(domain)) {
		...
		if (!pdb_get_trusteddom_pw(domain, ret_pwd, NULL,
					   &last_set_time))
		{
			DEBUG(1, ("get_trust_pw: could not fetch trust "
