...
static ssize_t client_receive_smb(struct cli_state *cli, size_t maxlen)
{
	...
	if (cli_encryption_on(cli)) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG(1, ("SMB decryption failed on incoming packet! Error %s\n",
				nt_errstr(status)));
...
bool cli_receive_smb(struct cli_state *cli)
{
 ...
 again:
	...
	if (!cli_check_sign_mac(cli, cli->inbuf)) {
		...
		DEBUG(1, ("SMB Signature verification failed on incoming packet!\n"));
		cli->smb_rw_error = SMB_READ_BAD_SIG;
		close(cli->fd);
		cli->fd = -1;
		return false;
...
bool cli_send_smb(struct cli_state *cli)
{
	...
	if (enc_on) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			...
			DEBUG(1,("Error in encrypting client message. Error %s\n",
				nt_errstr(status) ));
			return false;
		}
	}
				...
				size_t extradata)
{
	...
	if (client_is_signing_on(cli)) {
		DEBUG(1,("cli_send_smb_large: cannot send signed packet.\n"));
		return false;
	}
	...
	if (nwritten < (len + extradata)) {
		...
		cli->fd = -1;
		cli->smb_rw_error = SMB_WRITE_ERROR;
		DEBUG(1,("Error writing %d bytes to client. (%s)\n",
...
struct cli_state *cli_initialise(void)
{
	...
	if (is_setuid_root()) {
		DEBUG(1,("libsmb based programs must *NOT* be setuid root.\n"));
		return NULL;
	}
...
bool cli_send_keepalive(struct cli_state *cli)
{
        ...
        if (!send_keepalive(cli->fd)) {
                close(cli->fd);
                cli->fd = -1;
                DEBUG(1,("Error sending keepalive packet to client.\n"));
                return false;
        }
        return true;
}
