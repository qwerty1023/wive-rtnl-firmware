...
static void display_finfo(file_info *finfo, const char *dir)
{
	...
	if (!showacls) {
		...
		if (fnum == -1) {
			DEBUG(1, ("display_finfo() Failed to open %s: %s\n",
				afname,
			...
			if (!sd) {
				DEBUG(1, ("display_finfo() failed to "
			...
			if (temp_list->isdir) {
				...
				if (prompt && !yesno(quest)) { /* No */
					...
					if (!cli_chkpath(cli, rname) &&
					    !do_mkdir(rname)) {
						DEBUG(1, ("Unable to make dir, skipping..."));
						/* Skip the directory */
						lname[strlen(lname)-1] = '/';
						if (!seek_list(temp_list, lname)) {
							break;
 ...
 again:
	...
	*/
	if (FD_ISSET(cli->fd,&fds)) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG(1, ("Read from server failed, maybe it closed "
				  "the connection\n"));
	...
	/* Ping the server to keep the connection alive using SMBecho. */
	{
		...
		if (!cli_echo(cli, 1, garbage, sizeof(garbage))) {
			DEBUG(1, ("SMBecho failed. Maybe server has closed "
				  "the connection\n"));
			smb_readline_done();
			finished = true;
		}
