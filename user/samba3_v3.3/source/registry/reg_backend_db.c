...
WERROR init_registry_key(const char *add_path)
{
	...
	if (regdb->transaction_start(regdb) != 0) {
		DEBUG(1, ("init_registry_key: transaction_start failed\n"));
	...
	if (regdb->transaction_commit(regdb) != 0) {
		DEBUG(1, ("init_registry_key: Could not commit transaction\n"));
		return WERR_REG_IO_FAILURE;
	}
...
do_init:
	...
	if (regdb->transaction_start(regdb) != 0) {
		DEBUG(1, ("init_registry_data: tdb_transaction_start "
			  "failed\n"));
	...
	for (i=0; builtin_registry_values[i].path != NULL; i++) {
					...
					builtin_registry_values[i].valuename))
		{
			...
			default:
				DEBUG(1, ("init_registry_data: invalid value "
	...
	if (regdb->transaction_commit(regdb) != 0) {
		DEBUG(1, ("init_registry_data: Could not commit "
			  "transaction\n"));
		werr = WERR_REG_IO_FAILURE;
	} else {
		werr = WERR_OK;
...
WERROR regdb_open( void )
{
	...
	if ( !regdb ) {
		...
		DEBUG(1,("regdb_open: Failed to open %s! (%s)\n", 
			state_path("registry.tdb"), strerror(errno) ));
	}
	...
	for (i=0; i<num_subkeys; i++) {
		...
		if (len+thistime > buflen) {
			...
			buffer = SMB_REALLOC_ARRAY(buffer, uint8_t,
						   (len+thistime)*2);
			if(buffer == NULL) {
				DEBUG(1, ("regdb_store_keys: Failed to realloc "
					  "memory of size [%u]\n",
...
bool regdb_store_keys(const char *key, struct regsubkey_ctr *ctr)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG(1,("regdb_store_keys: talloc() failure!\n"));
		return false;
	}
	...
	if (regdb->transaction_start(regdb) != 0) {
		DEBUG(1, ("regdb_store_keys: transaction_start failed\n"));
		goto fail;
	}
	...
	if (!regdb_store_keys_internal(key, ctr) ) {
		DEBUG(1,("regdb_store_keys: Failed to store new subkey list "
			 "for parent [%s]\n", key));
		goto cancel;
	}
	...
	if (num_subkeys == 0) {
		...
		if (!regdb_store_keys_internal(key, subkeys)) {
			DEBUG(1,("regdb_store_keys: Failed to store "
	...
	if (regdb->transaction_commit(regdb) != 0) {
		DEBUG(1, ("regdb_store_keys: Could not commit transaction\n"));
		goto fail;
	}
...
static WERROR regdb_create_subkey(const char *key, const char *subkey)
{
	...
	if (!regdb_store_keys_internal(key, subkeys)) {
		DEBUG(1, (__location__ " failed to store new subkey list for "
			 "parent key %s\n", key));
		werr = WERR_REG_IO_FAILURE;
		goto cancel;
	}
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG(1, (__location__ " failed to commit transaction: %s\n",
			 dos_errstr(werr)));
	}
...
cancel:
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG(1, (__location__ " failed to cancel transaction: %s\n",
			 dos_errstr(werr)));
	}
...
static WERROR regdb_delete_subkey(const char *key, const char *subkey)
{
	...
	if (!regdb_store_keys_internal(key, subkeys)) {
		DEBUG(1, (__location__ " failed to store new subkey_list for "
			 "parent key %s\n", key));
		werr = WERR_REG_IO_FAILURE;
		goto cancel;
	}
...
static bool regdb_key_is_base_key(const char *key)
{
	...
	path = normalize_reg_path(mem_ctx, key);
	if (path == NULL) {
		DEBUG(1, ("out of memory! (talloc failed)\n"));
		goto done;
	}
...
static bool create_sorted_subkeys(const char *key, const char *sorted_keyname)
{
	...
	if (regdb->transaction_start(regdb) != 0) {
		DEBUG(1, ("create_sorted_subkeys: transaction_start "
			  "failed\n"));
		return false;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		if (regdb->transaction_cancel(regdb) == -1) {
			DEBUG(1, ("create_sorted_subkeys: transaction_cancel "
				  "failed\n"));
		}
		TALLOC_FREE(ctr);
		return false;
...
bool regdb_store_values( const char *key, REGVAL_CTR *values )
{
	...
	if (len <= 0) {
		DEBUG(1,("regdb_store_values: unable to pack values. len <= 0\n"));
		goto done;
	}
