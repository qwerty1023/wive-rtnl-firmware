	...
	for ( i=0; i<num_keys; i++ ) {
		...
		if ( snum == -1 ) {
			...
			if ( !add_printer_hook(talloc_tos(), NULL, &printer ) ) {
				DEBUG(1,("add_printers_by_registry: Failed to add printer [%s]\n",
					printername));
			}	
		}
	}
...
static bool key_printers_store_keys( const char *key, struct regsubkey_ctr *subkeys )
{
	...
	if ( !W_ERROR_IS_OK(get_a_printer(NULL, &printer, 2, printername)) ) {
		DEBUG(1,("key_printers_store_keys: Tried to store subkey for bad printername %s\n", 
			printername));
		return False;
	}
	...
	if ( !keyname ) {
		...
		for ( i=0; i<num_values; i++ ) {
			...
			if ( !W_ERROR_IS_OK(result) ) {
				DEBUG(1,("key_printers_store_values: failed to set printer data [%s]!\n",
					keyname));
				free_a_printer( &printer, 2 );
				return False;
			}
