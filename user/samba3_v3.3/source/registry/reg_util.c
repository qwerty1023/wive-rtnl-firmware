...
int regval_convert_multi_sz( uint16 *multi_string, size_t byte_len, char ***values )
{
	...
	*values = NULL;
	...
	if ( !(sz = TALLOC_ARRAY( NULL, char*, num_strings+1 )) ) {
		DEBUG(1,("reg_convert_multi_sz: talloc() failed!\n"));
		return -1;
	}
...
size_t regval_build_multi_sz( char **values, uint16 **buffer )
{
	...
	if ( !(buf = TALLOC_ARRAY( NULL, uint16, 2 )) ) {
		DEBUG(1,("regval_build_multi_sz: talloc() failed!\n"));
		return 0;
	}

	for ( i=0; values[i]; i++ ) {
		ZERO_STRUCT( sz );
		/* DEBUG(1,("regval_build_multi_sz: building [%s]\n",values[i])); */
		...
		if ( !b ) {
			DEBUG(1,("regval_build_multi_sz: talloc() reallocation error!\n"));
			TALLOC_FREE( buffer );
			return 0;
		}
		buf = b;
