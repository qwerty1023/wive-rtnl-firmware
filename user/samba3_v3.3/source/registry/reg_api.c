				   ...
				   struct registry_key **pregkey)
{
	...
	if ( !(key->ops = reghook_cache_find( key->name )) ) {
		DEBUG(1,("reg_open_onelevel: Failed to assign "
			 "REGISTRY_OPS to [%s]\n", key->name ));
		result = WERR_BADFILE;
		goto done;
	}
			    ...
			    REGF_NK_REC *key)
{
	...
	if (!registry_key.ops) {
		DEBUG(1, ("reg_load_tree: Failed to assign  REGISTRY_OPS "
			  "to [%s]\n", topkeypath));
		return WERR_BADFILE;
	}
	...
	if (!registry_key.name) {
		DEBUG(1, ("reg_load_tree: Talloc failed for reg_key.name!\n"));
		return WERR_NOMEM;
	}
	...
	{
		DEBUG(1,("reg_load_tree: Failed to load %s!\n", topkeypath));
...
static WERROR restore_registry_key(REGISTRY_KEY *krecord, const char *fname)
{
	...
	if (regfile == NULL) {
		DEBUG(1, ("restore_registry_key: failed to open \"%s\" (%s)\n",
...
static WERROR backup_registry_key(REGISTRY_KEY *krecord, const char *fname)
{
	...
	regfile = regfio_open(fname, (O_RDWR|O_CREAT|O_EXCL),
			      (S_IREAD|S_IWRITE));
	if (regfile == NULL) {
		DEBUG(1,("backup_registry_key: failed to open \"%s\" (%s)\n",
			 fname, strerror(errno) ));
		return ntstatus_to_werror(map_nt_error_from_unix(errno));
	}
					    ...
					    bool del_key)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG(1, ("reg_deletekey_recursive_trans: "
