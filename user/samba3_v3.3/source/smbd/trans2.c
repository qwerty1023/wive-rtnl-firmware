...
static struct ea_list *read_ea_name_list(TALLOC_CTX *ctx, const char *pdata, size_t data_size)
{
	...
	while (offset + 2 < data_size) {
		...
		if (!pull_ascii_talloc(ctx, &eal->ea.name, &pdata[offset],
				       &converted_size)) {
			DEBUG(1,("read_ea_name_list: pull_ascii_talloc "
...
struct ea_list *read_ea_list_entry(TALLOC_CTX *ctx, const char *pdata, size_t data_size, size_t *pbytes_used)
{
	...
	if (!pull_ascii_talloc(ctx, &eal->ea.name, pdata + 4, &converted_size)) {
		DEBUG(1,("read_ea_list_entry: pull_ascii_talloc failed: %s",
			 strerror(errno)));
	}
	if (!eal->ea.name) {
		return NULL;
			 ...
			 int max_data_bytes)
{
	...
	if (useable_space < 0) {
		DEBUG(1, ("send_trans2_replies failed sanity useable_space "
			  "= %d!!!", useable_space));
	...
	while (params_to_send || data_to_send) {
		...
		/* Sanity check */
		if(params_to_send < 0 || data_to_send < 0) {
			DEBUG(1,("send_trans2_replies failed sanity check pts = %d, dts = %d\n!!!",
...
static uint32 unix_filetype(mode_t mode)
{
	...
	DEBUG(1,("unix_filetype: unknown filetype %u\n", (unsigned)mode));
			       ...
			       unsigned int max_data_bytes)
{
	...
	if (IS_IPC(conn)) {
		if (info_level != SMB_QUERY_CIFS_UNIX_INFO) {
			DEBUG(1,("call_trans2qfsinfo: not an allowed "
	...
	if (ENCRYPTION_REQUIRED(conn) && !req->encrypted) {
		if (info_level != SMB_QUERY_CIFS_UNIX_INFO) {
			DEBUG(1,("call_trans2qfsinfo: encryption required "
...
cBytesSector=%u, cUnitTotal=%u, cUnitAvail=%d\n", (unsigned int)bsize, (unsigned int)sectors_per_unit,
		 ...
		 */
		{
			...
			if (conn->server_info->utok.uid != 0) {
				DEBUG(1,("set_user_quota: access_denied "
					 "service [%s] user [%s]\n",
			...
			if (vfs_get_ntquota(&fsp, SMB_USER_FS_QUOTA_TYPE, NULL, &quotas)!=0) {
				DEBUG(1,("vfs_get_ntquota() failed for service [%s]\n",lp_servicename(SNUM(conn))));
		...
		case SMB_QUERY_POSIX_FS_INFO:
		{
			...
			if (!rc) {
				...
				DEBUG(1,("vfs_statvfs() failed for service [%s]\n",lp_servicename(SNUM(conn))));
				reply_doserror(req, ERRSRV, ERRerror);
				return;
			}
			break;
				 ...
				 unsigned int max_data_bytes)
{
	...
	/*  */
	if (total_params < 4) {
		DEBUG(1,("call_trans2setfsinfo: requires total_params(%d) >= 4 bytes!\n",
			total_params));
		reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
		return;
	}
	...
	if (IS_IPC(conn)) {
		if (info_level != SMB_REQUEST_TRANSPORT_ENCRYPTION &&
			...
			DEBUG(1,("call_trans2setfsinfo: not an allowed "
				"info level (0x%x) on IPC$.\n",
				(unsigned int)info_level));
			reply_nterror(req, NT_STATUS_ACCESS_DENIED);
			return;
	...
	if (ENCRYPTION_REQUIRED(conn) && !req->encrypted) {
		if (info_level != SMB_REQUEST_TRANSPORT_ENCRYPTION) {
			DEBUG(1,("call_trans2setfsinfo: encryption required "
				"and info level 0x%x sent.\n",
				(unsigned int)info_level));
			exit_server_cleanly("encryption required "
				"on connection");
		...
		case SMB_FS_QUOTA_INFORMATION:
			{
				...
				if (total_data < 42) {
					DEBUG(1,("call_trans2setfsinfo: SET_FS_QUOTA: requires total_data(%d) >= 42 bytes!\n",
						total_data));
					reply_nterror(
						req,
						NT_STATUS_INVALID_PARAMETER);
...
static bool marshall_posix_acl(connection_struct *conn, char *pdata, SMB_STRUCT_STAT *pst, SMB_ACL_T posix_acl)
{
	...
	while ( posix_acl && (SMB_VFS_SYS_ACL_GET_ENTRY(conn, posix_acl, entry_id, &entry) == 1)) {
		...
		if (SMB_VFS_SYS_ACL_GET_TAG_TYPE(conn, entry, &tagtype) == -1) {
			DEBUG(1,("marshall_posix_acl: SMB_VFS_SYS_ACL_GET_TAG_TYPE failed.\n"));
			return False;
		}

		if (SMB_VFS_SYS_ACL_GET_PERMSET(conn, entry, &permset) == -1) {
			DEBUG(1,("marshall_posix_acl: SMB_VFS_SYS_ACL_GET_PERMSET failed.\n"));
			return False;
		}
		...
		switch (tagtype) {
			...
			case SMB_ACL_USER:
				{
					...
					if (!puid) {
						DEBUG(1,("marshall_posix_acl: SMB_VFS_SYS_ACL_GET_QUALIFIER failed.\n"));
						return False;
					}
			...
			case SMB_ACL_OTHER:
				...
				break;
			default:
				DEBUG(1,("marshall_posix_acl: unknown tagtype.\n"));
				return False;
		}
				...
				SMB_STRUCT_STAT *psbuf)
{
	...
	lck = get_share_mode_lock(talloc_tos(), fsp->file_id, NULL, NULL,
				  NULL);
	if (lck == NULL) {
		DEBUG(1, ("smb_posix_unlink: Could not get share mode "
			"lock for file %s\n", fsp->fsp_name));
		close_file(fsp, NORMAL_CLOSE);
		return NT_STATUS_INVALID_PARAMETER;
	}
			  ...
			  struct trans_state *state)
{
	...
	if (conn->encrypt_level == Required && !req->encrypted) {
		if (state->call != TRANSACT2_QFSINFO &&
			...
			DEBUG(1,("handle_trans2: encryption required "
				"with call 0x%x\n",
...
void reply_trans2(struct smb_request *req)
{
	...
	if (state->total_data) {
		...
		if (state->data == NULL) {
			DEBUG(1,("reply_trans2: data malloc fail for %u "
	...
	if (state->total_param) {
		...
		if (state->param == NULL) {
			DEBUG(1,("reply_trans: param malloc fail for %u "
  ...
  bad_param:
	...
	DEBUG(1,("reply_trans2: invalid trans parameters\n"));
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
	END_PROFILE(SMBtrans2);
...
void reply_transs2(struct smb_request *req)
{
  ...
  bad_param:
	...
	DEBUG(1,("reply_transs2: invalid trans parameters\n"));
	DLIST_REMOVE(conn->pending_trans, state);
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
