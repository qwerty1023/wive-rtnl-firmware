...
bool srv_send_smb(int fd, char *buffer, bool do_encrypt)
{
	...
	if (do_encrypt) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG(1, ("send_smb: SMB encryption failed "
						    ...
						    size_t *len_ret)
{
	...
	if (is_valid_writeX_buffer((uint8_t *)writeX_header)) {
		...
		*buffer = (char *)TALLOC_MEMDUP(mem_ctx,
		...
		if (*buffer == NULL) {
			DEBUG(1, ("Could not allocate inbuf of length %d\n",
				   ...
				   size_t *p_len)
{
	...
	*p_encrypted = false;
	...
	if (is_encrypted_packet((uint8_t *)*buffer)) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG(1, ("receive_smb_talloc: SMB decryption failed on "
	...
	if (!srv_check_sign_mac(*buffer, true)) {
		DEBUG(1, ("receive_smb: SMB Signature verification failed on "
			  "incoming packet!\n"));
			...
			bool encrypted)
{
	...
	if (req_size < smb_size) {
		DEBUG(1,("init_smb_request: invalid request size %u\n",
	...
	if (smb_size + req->wct*2 > req_size) {
		DEBUG(1,("init_smb_request: invalid wct number %u (size %u)\n",
	...
	if (((uint8 *)smb_buf(inbuf)) + smb_buflen(inbuf) > inbuf + req_size) {
		DEBUG(1,("init_smb_request: invalid bcc number %u "
			"(wct = %u, size %u)\n",
				...
				char *private_data, size_t private_len)
{
	...
	if(msg == NULL) {
		DEBUG(1,("push_message: malloc fail (1)\n"));
		return False;
	}

	msg->buf = data_blob_talloc(msg, req->inbuf, msg_len);
	if(msg->buf.data == NULL) {
		DEBUG(1,("push_message: malloc fail (2)\n"));
		TALLOC_FREE(msg);
		return False;
	}
	...
	if (private_data) {
		...
		if (msg->private_data.data == NULL) {
			DEBUG(1,("push_message: malloc fail (3)\n"));
			TALLOC_FREE(msg);
			return False;
		}
	}
	...
	for (pml = deferred_open_queue; pml; pml = pml->next) {
		...
		if (mid == msg_mid) {
			...
			if (pml->processed) {
				 ...
				 * rescheduled. */
				DEBUG(1,("schedule_deferred_open_smb_message: LOGIC ERROR "
			       ...
			       char *private_data, size_t priv_len)
{
	...
	if (req->unread_bytes) {
		DEBUG(1,("push_deferred_smb_message: logic error ! "
			"unread_bytes = %u\n",
				  ...
				  void *private_data)
{
	...
	if (result->te == NULL) {
		DEBUG(1, ("event_add_timed failed\n"));
		TALLOC_FREE(result);
		return NULL;
	}
...
void respond_to_all_remaining_local_messages(void)
{
	...
	if(get_number_of_exclusive_open_oplocks()) {
		DEBUG(1,("respond_to_all_remaining_local_messages: PANIC : we have %d exclusive oplocks.\n",
...
static void smb_dump(const char *name, int type, const char *data, ssize_t len)
{
	...
	if (fd != -1) {
		...
		if (ret != len)
			DEBUG(1,("smb_dump: problem: write returned %d\n", (int)ret ));
		close(fd);
		DEBUG(1,("created %s len %lu\n", fname, (unsigned long)len));
	}
	SAFE_FREE(fname);
}
...
static connection_struct *switch_message(uint8 type, struct smb_request *req, int size)
{
	...
	if (smb_messages[type].fn == NULL) {
		DEBUG(1,("Unknown message type %d!\n",type));
...
void chain_reply(struct smb_request *req)
{
	...
	if (new_size < 0) {
		DEBUG(1,("chain_reply: chain packet size incorrect "
