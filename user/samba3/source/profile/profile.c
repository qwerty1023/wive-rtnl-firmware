			    ...
			    DATA_BLOB *data)
{
	...
	if (data->length != sizeof(level)) {
		DEBUG(1, ("got invalid profile message\n"));
		return;
	}
...
bool profile_setup(struct messaging_context *msg_ctx, bool rdonly)
{
 ...
 again:
	...
	if (shm_id == -1) {
		DEBUG(1,("Can't create or use IPC area. Error was %s\n", 
			 strerror(errno)));
		return False;
	}   
	...
	if ((long)profile_h == -1) {
		DEBUG(1,("Can't attach to IPC area. Error was %s\n", 
			 strerror(errno)));
		return False;
	}
	...
	if (shmctl(shm_id, IPC_STAT, &shm_ds) != 0) {
		DEBUG(1,("ERROR shmctl : can't IPC_STAT. Error was %s\n", 
			 strerror(errno)));
		return False;
	}
	...
	if (shm_ds.shm_perm.cuid != sec_initial_uid() ||
		...
		DEBUG(1,("ERROR: we did not create the shmem "
	...
	if (shm_ds.shm_segsz != sizeof(*profile_h)) {
		DEBUG(1,("WARNING: profile size is %d (expected %d). Deleting\n",
