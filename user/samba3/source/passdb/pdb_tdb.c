...
static int tdbsam_convert_one(struct db_record *rec, void *priv)
{
	...
	if (user == NULL) {
		DEBUG(1,("tdbsam_convert: samu_new() failed!\n"));
		state->success = false;
		return -1;
	}
	...
	if (!ret) {
		DEBUG(1,("tdbsam_convert: Bad struct samu entry returned "
	...
	if (data.dsize == -1) {
		DEBUG(1,("tdbsam_convert: cannot pack the struct samu into "
			 "the new format\n"));
		state->success = false;
		return -1;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1, ("Could not store the new record: %s\n",
			  nt_errstr(status)));
		state->success = false;
		return -1;
	}
...
static bool tdbsam_convert_backup(const char *dbname, struct db_context **pp_db)
{
	...
	if (tmp_db == NULL) {
		DEBUG(1, ("tdbsam_convert_backup: Failed to create backup TDB passwd "
			  "[%s]\n", tmp_fname));
		TALLOC_FREE(frame);
		return false;
	}
	...
	if (orig_db->transaction_start(orig_db) != 0) {
		DEBUG(1, ("tdbsam_convert_backup: Could not start transaction (1)\n"));
		unlink(tmp_fname);
		TALLOC_FREE(tmp_db);
		TALLOC_FREE(frame);
		return false;
	...
	if (tmp_db->transaction_start(tmp_db) != 0) {
		DEBUG(1, ("tdbsam_convert_backup: Could not start transaction (2)\n"));
	...
	bs.success = true;
        ...
        if (ret < 0) {
                DEBUG(1, ("tdbsam_convert_backup: traverse failed\n"));
                goto cancel;
        }

	if (!bs.success) {
		DEBUG(1, ("tdbsam_convert_backup: Rewriting records failed\n"));
		goto cancel;
	}
	...
	if (rename(tmp_fname, dbname) == -1) {
		DEBUG(1, ("tdbsam_convert_backup: rename of %s to %s failed %s\n",
			tmp_fname,
			dbname,
			strerror(errno)));
	...
	orig_db = db_open(NULL, dbname, 0,
			  TDB_DEFAULT, O_CREAT|O_RDWR, 0600);
	if (orig_db == NULL) {
		DEBUG(1, ("tdbsam_convert_backup: Failed to re-open "
...
static bool tdbsam_convert(struct db_context **pp_db, const char *name, int32 from)
{
	...
	if (db_is_local(name) && !tdbsam_convert_backup(name, pp_db)) {
		DEBUG(1, ("tdbsam_convert: Could not backup %s\n", name));
		return false;
	}
	...
	if (db->transaction_start(db) != 0) {
		DEBUG(1, ("tdbsam_convert: Could not start transaction\n"));
		return false;
	}

	if (!tdbsam_upgrade_next_rid(db)) {
		DEBUG(1, ("tdbsam_convert: tdbsam_upgrade_next_rid failed\n"));
		goto cancel;
	}

	ret = db->traverse(db, tdbsam_convert_one, &state);
	if (ret < 0) {
		DEBUG(1, ("tdbsam_convert: traverse failed\n"));
		goto cancel;
	}

	if (!state.success) {
		DEBUG(1, ("tdbsam_convert: Converting records failed\n"));
		goto cancel;
	}

	if (dbwrap_store_int32(db, TDBSAM_VERSION_STRING,
			       TDBSAM_VERSION) != 0) {
		DEBUG(1, ("tdbsam_convert: Could not store tdbsam version\n"));
		goto cancel;
	}

	if (dbwrap_store_int32(db, TDBSAM_MINOR_VERSION_STRING,
			       TDBSAM_MINOR_VERSION) != 0) {
		DEBUG(1, ("tdbsam_convert: Could not store tdbsam minor version\n"));
		goto cancel;
	}

	if (db->transaction_commit(db) != 0) {
		DEBUG(1, ("tdbsam_convert: Could not commit transaction\n"));
		return false;
	}
...
static bool tdbsam_open( const char *name )
{
	...
	if (db_sam == NULL) {
		DEBUG(1, ("tdbsam_open: Failed to open/create TDB passwd "
			  "[%s]\n", name));
		return false;
	}
	...
	if (version > TDBSAM_VERSION) {
		...
		DEBUG(1, ("tdbsam_open: unknown version => %d\n", version));
		TALLOC_FREE(db_sam);
		return false;
	}
	...
	if ( version < TDBSAM_VERSION ||
		...
		if (!mtx) {
			DEBUG(1, ("tdbsam_open: failed to grab mutex.\n"));
			TALLOC_FREE(db_sam);
			return false;
		}
		...
		if ( version < TDBSAM_VERSION ||
			...
			if ( !tdbsam_convert(&db_sam, name, version) ) {
				DEBUG(1, ("tdbsam_open: Error when trying to convert "
					  "tdbsam [%s]\n",name));
				TALLOC_FREE(db_sam);
				TALLOC_FREE(mtx);
				return false;
				    ...
				    struct samu *user, const char *sname)
{
	...
	if ( !user ) {
		DEBUG(1,("pdb_getsampwnam: struct samu is NULL.\n"));
	...
	if ( !tdbsam_open( tdbsam_filename ) ) {
		DEBUG(1,("tdbsam_getsampwnam: failed to open %s!\n", tdbsam_filename));
	...
	if (!init_samu_from_buffer(user, SAMU_BUFFER_LATEST, data.dptr, data.dsize)) {
		DEBUG(1,("pdb_getsampwent: Bad struct samu entry returned from TDB!\n"));
		SAFE_FREE(data.dptr);
		return NT_STATUS_NO_MEMORY;
	}
				    ...
				    struct samu *user, uint32 rid)
{
	...
	if ( !user ) {
		DEBUG(1,("pdb_getsampwrid: struct samu is NULL.\n"));
		return nt_status;
	}
	...
	if ( !tdbsam_open( tdbsam_filename ) ) {
		DEBUG(1,("tdbsam_getsampwrid: failed to open %s!\n", tdbsam_filename));
		return NT_STATUS_ACCESS_DENIED;
	}
...
static bool tdb_delete_samacct_only( struct samu *sam_pass )
{
	...
	/* it's outaa here!  8^) */
	if ( !tdbsam_open( tdbsam_filename ) ) {
		DEBUG(1,("tdb_delete_samacct_only: failed to open %s!\n",
			 tdbsam_filename));
		return false;
	}
					  ...
					  struct samu *sam_pass)
{
	...
	if ( !tdbsam_open( tdbsam_filename ) ) {
		DEBUG(1,("tdbsam_delete_sam_account: failed to open %s!\n",
			 tdbsam_filename));
		return NT_STATUS_ACCESS_DENIED;
	}
	...
	if (db_sam->transaction_commit(db_sam) != 0) {
		DEBUG(1, ("Could not commit transaction\n"));
...
static bool tdb_update_samacct_only( struct samu* newpwd, int flag )
{
	...
	if ( (data.dsize=init_buffer_from_samu(&buf, newpwd, False)) == -1 ) {
		DEBUG(1,("tdb_update_sam: ERROR - Unable to copy struct samu info BYTE buffer!\n"));
		goto done;
	}
	data.dptr = buf;
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1, ("Unable to modify passwd TDB: %s!",
			  nt_errstr(status)));
		goto done;
	}
...
static bool tdb_update_ridrec_only( struct samu* newpwd, int flag )
{
	...
	status = dbwrap_store_bystring(db_sam, keystr, data, flag);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1, ("Unable to modify TDB passwd: %s!\n",
			  nt_errstr(status)));
		return false;
	}
			   ...
			   int flag)
{
	...
	if (!(newrid = pdb_get_user_rid(newpwd))) {
		DEBUG(1,("tdb_update_sam: struct samu (%s) with no RID!\n",
	...
	if (flag == TDB_MODIFY) {
		...
		if (account == NULL) {
			DEBUG(1,("tdb_update_sam: samu_new() failed\n"));
			goto cancel;
		}
		if (!NT_STATUS_IS_OK(tdbsam_getsampwnam(my_methods, account, pdb_get_username(newpwd)))) {
			DEBUG(1,("tdb_update_sam: tdbsam_getsampwnam() for %s failed\n",
				pdb_get_username(newpwd)));
			TALLOC_FREE(account);
			goto cancel;
		}
		if (!(oldrid = pdb_get_user_rid(account))) {
			DEBUG(1,("tdb_update_sam: pdb_get_user_rid() failed\n"));
			TALLOC_FREE(account);
			goto cancel;
		}
		TALLOC_FREE(account);
	...
	if (flag == TDB_MODIFY && newrid != oldrid) { 
		...
		if (!NT_STATUS_IS_OK(dbwrap_delete_bystring(db_sam, keystr))) {
			DEBUG(1, ("tdb_update_sam: Can't delete %s\n", keystr));
			goto cancel;
		}
		/* Insert new RID key */
					  ...
					  const char *newname)
{
	...
	if (db_sam->transaction_commit(db_sam) != 0) {
		 ...
		 * account back?
		 */
		DEBUG(1, ("transaction_commit failed\n"));
		TALLOC_FREE(new_acct);
		return NT_STATUS_INTERNAL_DB_CORRUPTION;	
	}
...
static bool tdbsam_new_rid(struct pdb_methods *methods, uint32 *prid)
{
	...
	if (!tdbsam_open(tdbsam_filename)) {
		DEBUG(1,("tdbsam_new_rid: failed to open %s!\n",
			tdbsam_filename));
		return false;
	}
