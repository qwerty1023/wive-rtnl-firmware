...
static const char* get_userattr_key2string( int schema_ver, int key )
{
	switch ( schema_ver ) {
		...
		default:
			DEBUG(1,("get_userattr_key2string: unknown schema version specified\n"));
			break;
	}
	return NULL;
}
...
const char** get_userattr_list( TALLOC_CTX *mem_ctx, int schema_ver )
{
	switch ( schema_ver ) {
		...
		default:
			DEBUG(1,("get_userattr_list: unknown schema version specified!\n"));
			break;
	}
					      ...
					      int schema_ver )
{
	switch ( schema_ver ) {
		...
		default:
			DEBUG(1,("get_userattr_delete_list: unknown schema version specified!\n"));
			break;
	}
...
static const char* get_objclass_filter( int schema_ver )
{
	...
	switch( schema_ver ) {
		...
		case SCHEMAVER_SAMBASAMACCOUNT:
			...
			break;
		default:
			DEBUG(1,("get_objclass_filter: Invalid schema version specified!\n"));
			objclass_filter[0] = '\0';
			break;
	}
...
static NTSTATUS ldapsam_get_seq_num(struct pdb_methods *my_methods, time_t *seq_num)
{
	...
	if (!next_token_talloc(mem_ctx, &p, &tok, "#")) {
		DEBUG(1,("ldapsam_get_seq_num: failed to parse sequence number\n"));
		goto done;
	}
				...
				LDAPMessage * entry)
{
	...
	if (sampass == NULL || ldap_state == NULL || entry == NULL) {
		DEBUG(1, ("init_sam_from_ldap: NULL parameters found!\n"));
		goto fn_exit;
	}

	if (priv2ld(ldap_state) == NULL) {
		DEBUG(1, ("init_sam_from_ldap: ldap_state->smbldap_state->"
			  "ldap_struct is NULL!\n"));
		goto fn_exit;
	}
	...
	if (ldap_state->is_nds_ldap) {
		...
		if (user_dn != NULL) {
			...
			DEBUG(1, ("init_sam_from_ldap: failed to get user_dn for '%s'\n", username));
		}
	}
	...
	if (pwHistLen > 0){
		...
		if ((pwhist = TALLOC_ARRAY(ctx, uint8,
				...
				NULL){
			DEBUG(1, ("init_sam_from_ldap: talloc failed!\n"));
			goto fn_exit;
		}
						    ...
						    enum pdb_elements))
{
	...
	if (mods == NULL || sampass == NULL) {
		DEBUG(1, ("init_ldap_from_sam: NULL parameters found!\n"));
		return False;
	}
	...
	*mods = NULL;
	...
	if (need_update(sampass, PDB_USERSID)) {
		...
		switch ( ldap_state->schema_ver ) {
			...
			default:
				DEBUG(1,("init_ldap_from_sam: unknown schema version specified\n"));
				break;
		}
	}
				     ...
				     bool (*need_update)(const struct samu *, enum pdb_elements))
{
	...
	if (!mods) {
		...
		switch(ldap_op) {
			...
			case LDAP_MOD_REPLACE: 
				...
				break;
			default: 	
				DEBUG(1,("ldapsam_modify_entry: Wrong LDAP operation type: %d!\n", 
					 ldap_op));
				return NT_STATUS_INVALID_PARAMETER;
		}
	...
	if (!(pdb_get_acct_ctrl(newpwd)&(ACB_WSTRUST|ACB_SVRTRUST|ACB_DOMTRUST)) &&
		...
		if ((ber = ber_alloc_t(LBER_USE_DER))==NULL) {
			DEBUG(1,("ber_alloc_t returns NULL\n"));
			SAFE_FREE(utf8_password);
			SAFE_FREE(utf8_dn);
			return NT_STATUS_UNSUCCESSFUL;
		}
		...
		if ((ber_printf (ber, "{") < 0) ||
		    (ber_printf (ber, "ts", LDAP_TAG_EXOP_MODIFY_PASSWD_ID,
				 utf8_dn) < 0)) {
			DEBUG(1,("ldapsam_modify_entry: ber_printf returns a "
				 "value <0\n"));
			ber_free(ber,1);
			SAFE_FREE(utf8_dn);
			SAFE_FREE(utf8_password);
		...
		if (ret < 0) {
			...
			DEBUG(1,("ldapsam_modify_entry: ber_flatten returns a value <0\n"));
			ber_free(ber,1);
			SAFE_FREE(utf8_dn);
			SAFE_FREE(utf8_password);
			return NT_STATUS_UNSUCCESSFUL;
		...
		if (rc != LDAP_SUCCESS) {
			...
			ldap_get_option(ldap_state->smbldap_state->ldap_struct, LDAP_OPT_ERROR_STRING,
					&ld_error);
			DEBUG(1,("ldapsam_modify_entry: LDAP Password could not be changed for user %s: %s\n\t%s\n",
					   ...
					   struct samu * sam_acct)
{
	...
	if (!sam_acct) {
		DEBUG(1, ("ldapsam_delete_sam_account: sam_acct was NULL!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
...
static NTSTATUS ldapsam_update_sam_account(struct pdb_methods *my_methods, struct samu * newpwd)
{
	...
	if (ldap_count_entries(ldap_state->smbldap_state->ldap_struct, result) == 0) {
		DEBUG(1, ("ldapsam_update_sam_account: No user to modify!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	if (!init_ldap_from_sam(ldap_state, entry, &mods, newpwd,
				element_is_changed)) {
		DEBUG(1, ("ldapsam_update_sam_account: init_ldap_from_sam failed!\n"));
		SAFE_FREE(dn);
		if (mods != NULL)
					   ...
					   const char *newname)
{
	...
	if (!old_acct) {
		DEBUG(1, ("ldapsam_rename_sam_account: old_acct was NULL!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
	if (!newname) {
		DEBUG(1, ("ldapsam_rename_sam_account: newname was NULL!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
...
static NTSTATUS ldapsam_add_sam_account(struct pdb_methods *my_methods, struct samu * newpwd)
{
	...
	if (!username || !*username) {
		DEBUG(1, ("ldapsam_add_sam_account: Cannot add user without a username!\n"));
	...
	if (ldap_count_entries(ldap_state->smbldap_state->ldap_struct, result) != 0) {
		DEBUG(1,("ldapsam_add_sam_account: User '%s' already in the base, with samba attributes\n", 
			 username));
		goto fn_exit;
	}
	ldap_msgfree(result);
	...
	if (element_is_set_or_changed(newpwd, PDB_USERSID)) {
		...
		if (rc == LDAP_SUCCESS) {
			if (ldap_count_entries(ldap_state->smbldap_state->ldap_struct, result) != 0) {
				DEBUG(1,("ldapsam_add_sam_account: SID '%s' "
	...
	if (num_result > 1) {
		DEBUG(1, ("ldapsam_add_sam_account: More than one user with that uid exists: bailing out!\n"));
		goto fn_exit;
	}
	...
	if (num_result == 1) {
		...
		if (num_result > 1) {
			DEBUG(1, ("ldapsam_add_sam_account: More than one user with specified Sid exists: bailing out!\n"));
			goto fn_exit;
		}
	...
	if (!init_ldap_from_sam(ldap_state, entry, &mods, newpwd,
		...
		DEBUG(1, ("ldapsam_add_sam_account: init_ldap_from_sam failed!\n"));
		if (mods != NULL) {
			ldap_mods_free(mods, true);
		}
		goto fn_exit;
	...
	if (mods == NULL) {
		DEBUG(1,("ldapsam_add_sam_account: mods is empty: nothing to add for user: %s\n",pdb_get_username(newpwd)));
		goto fn_exit;
	}
	switch ( ldap_state->schema_ver ) {
		case SCHEMAVER_SAMBAACCOUNT:
		...
		case SCHEMAVER_SAMBASAMACCOUNT:
			...
			break;
		default:
			DEBUG(1,("ldapsam_add_sam_account: invalid schema version specified\n"));
			break;
	}

	ret = ldapsam_modify_entry(my_methods,newpwd,dn,mods,ldap_op, element_is_set_or_changed);
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG(1,("ldapsam_add_sam_account: failed to modify/add user with uid = %s (dn = %s)\n",
			 pdb_get_username(newpwd),dn));
		ldap_mods_free(mods, true);
		goto fn_exit;
	}
				 ...
				 GROUP_MAP *map, LDAPMessage *entry)
{
	...
	if (ldap_state == NULL || map == NULL || entry == NULL ||
		...
		DEBUG(1, ("init_group_from_ldap: NULL parameters found!\n"));
		TALLOC_FREE(ctx);
		return false;
	}
	...
	temp = smbldap_talloc_single_attribute(
			...
			ctx);
	if (!temp) {
		DEBUG(1, ("init_group_from_ldap: Mandatory attribute %s not found\n", 
	...
	if ((map->sid_name_use < SID_NAME_USER) ||
		...
		DEBUG(1, ("init_group_from_ldap: Unknown Group type: %d\n", map->sid_name_use));
		TALLOC_FREE(ctx);
		return false;
	}
	...
	if (!temp) {
		temp = smbldap_talloc_single_attribute(
				...
				ctx);
		if (!temp) {
			DEBUG(1, ("init_group_from_ldap: Attributes cn not found either \
					   ...
					   size_t *p_num_members)
{
	...
	*p_num_members = 0;
	...
	if (!gidstr) {
		DEBUG(1, ("ldapsam_enum_group_members: Unable to find the group's gid!\n"));
		ret = NT_STATUS_INTERNAL_DB_CORRUPTION;
		goto done;
	}
	...
	if (values) {
		     ...
		     entry = ldap_next_entry(conn->ldap_struct, entry))
		{
			...
			sidstr = smbldap_talloc_single_attribute(conn->ldap_struct,
								 ...
								 mem_ctx);
			if (!sidstr) {
				DEBUG(1, ("Severe DB error, %s can't miss the sambaSID"
	     ...
	     entry = ldap_next_entry(conn->ldap_struct, entry))
	{
		...
		if (!ldapsam_extract_rid_from_entry(conn->ldap_struct,
						    ...
						    get_global_sam_sid(),
						    &rid)) {
			DEBUG(1, ("Severe DB error, %s can't miss the samba SID"								"attribute\n", LDAP_OBJ_SAMBASAMACCOUNT));
			ret = NT_STATUS_INTERNAL_DB_CORRUPTION;
			goto done;
		}
				    ...
				    bool update)
{
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG(1, ("ldapsam_setsamgrent: LDAP search failed: %s\n",
			  ldap_err2string(rc)));
					   ...
					   bool unix_only)
{
	...
	*pp_rmap = NULL;
	...
	if (!NT_STATUS_IS_OK(ldapsam_setsamgrent(methods, False))) {
		DEBUG(1, ("ldapsam_enum_group_mapping: Unable to open "
			  "passdb\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
	...
	while (NT_STATUS_IS_OK(ldapsam_getsamgrent(methods, &map))) {
		...
		if (!(*pp_rmap)) {
			DEBUG(1,("ldapsam_enum_group_mapping: Unable to "
				 "enlarge group map!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
						   ...
						   uint32 value)
{
	...
	if (policy_attr == NULL) {
		DEBUG(1,("ldapsam_set_account_policy_in_ldap: invalid "
			 "policy\n"));
		return ntstatus;
	}
	...
	if (!cache_account_policy_set(policy_index, value)) {
		DEBUG(1,("ldapsam_set_account_policy_in_ldap: failed to "
						     ...
						     uint32 *value)
{
	...
	policy_attr = get_account_policy_attr(policy_index);
	if (!policy_attr) {
		DEBUG(1,("ldapsam_get_account_policy_from_ldap: invalid "
			 "policy index: %d\n", policy_index));
		return ntstatus;
	}
...
/* update_ldap: */
 ...
 update_cache:
	...
	if (!cache_account_policy_set(policy_index, *value)) {
		DEBUG(1,("ldapsam_get_account_policy: failed to update local "
			 "tdb as a cache\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
				  ...
				  struct samr_displayentry *result)
{
	...
	{
		DEBUG(1,("ldapuser2displayentry: pull_utf8_talloc failed: %s",
			 strerror(errno)));
	}
	...
	if ((vals == NULL) || (vals[0] == NULL)) {
		DEBUG(1, ("\"objectSid\" not found\n"));
		return False;
	}

	if (!string_to_sid(&sid, vals[0])) {
		DEBUG(1, ("Could not convert %s to SID\n", vals[0]));
		ldap_value_free(vals);
		return False;
	}
	ldap_value_free(vals);
	...
	if (!sid_peek_check_rid(get_global_sam_sid(), &sid, &result->rid)) {
		DEBUG(1, ("sid %s does not belong to our domain\n",
			  sid_string_dbg(&sid)));
		return False;
	}
				   ...
				   struct samr_displayentry *result)
{
	...
	if ((vals == NULL) || (vals[0] == NULL)) {
		...
		if (!pull_utf8_talloc(mem_ctx,
				      ...
				      vals[0], &converted_size))
		{
			DEBUG(1,("ldapgroup2displayentry: pull_utf8_talloc "
				  "failed: %s", strerror(errno)));
				   ...
				   vals[0], &converted_size))
	{
		DEBUG(1,("ldapgroup2displayentry: pull_utf8_talloc failed: %s",
			  strerror(errno)));
	}
	...
	switch (group_type) {
		...
		case SID_NAME_ALIAS:
			...
			{
				DEBUG(1, ("%s is not in our domain\n",
					  sid_string_dbg(&sid)));
				return False;
			}
			break;
				    ...
				    uint32 *rid)
{
	...
	if (entry == NULL) {
		DEBUG(1, ("Could not get domain info entry\n"));
				    ...
				    uint32 acb_info, uint32 *rid)
{
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG(1,("ldapsam_create_user: ldap search failed!\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
	...
	if (num_result > 1) {
		DEBUG(1, ("ldapsam_create_user: More than one user with name [%s] ?!\n", name));
	...
	if (num_result == 1) {
		...
		if (!dn) {
			DEBUG(1,("ldapsam_create_user: Out of memory!\n"));
			return NT_STATUS_NO_MEMORY;
		}
	}
	...
	if (add_posix) {
		...
		if (!sid_compose(&group_sid, get_global_sam_sid(), DOMAIN_GROUP_RID_USERS) ||
			...
			DEBUG(1, ("ldapsam_create_user: Unable to get the Domain Users gid: bailing out!\n"));
			return NT_STATUS_INVALID_PRIMARY_GROUP;
		}

		/* lets allocate a new userid for this user */
		if (!winbind_allocate_uid(&uid)) {
			DEBUG(1, ("ldapsam_create_user: Unable to allocate a new user id: bailing out!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG(1,("ldapsam_create_user: failed to create a new user [%s] (dn = %s)\n", name ,dn));
		return NT_STATUS_UNSUCCESSFUL;
	}
...
static NTSTATUS ldapsam_delete_user(struct pdb_methods *my_methods, TALLOC_CTX *tmp_ctx, struct samu *sam_acct)
{
	...
	DEBUG(1,("ldapsam_delete_user: Attempt to delete user [%s]\n", pdb_get_username(sam_acct)));
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG(1,("ldapsam_delete_user: user search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result == 0) {
		DEBUG(1,("ldapsam_delete_user: user not found!\n"));
		return NT_STATUS_NO_SUCH_USER;
	}

	if (num_result > 1) {
		DEBUG(1, ("ldapsam_delete_user: More than one user with name [%s] ?!\n", pdb_get_username(sam_acct)));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	dn = smbldap_talloc_dn(tmp_ctx, priv2ld(ldap_state), entry);
	if (!dn) {
		DEBUG(1,("ldapsam_delete_user: Out of memory!\n"));
		return NT_STATUS_NO_MEMORY;
	}
					 ...
					 uint32 *rid)
{
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG(1,("ldapsam_create_group: ldap search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result > 1) {
		DEBUG(1, ("ldapsam_create_group: There exists more than one group with name [%s]: bailing out!\n", name));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	if (num_result == 1) {
		...
		dn = smbldap_talloc_dn(tmp_ctx, priv2ld(ldap_state), entry);
		if (!dn) {
			DEBUG(1,("ldapsam_create_group: Out of memory!\n"));
			return NT_STATUS_NO_MEMORY;
		}
	}
	...
	if (num_result == 0) {
		...
		if (!winbind_allocate_gid(&gid)) {
			DEBUG(1, ("ldapsam_create_group: Unable to allocate a new group id: bailing out!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG(1,("ldapsam_create_group: failed to create a new group [%s] (dn = %s)\n", name ,dn));
		return NT_STATUS_UNSUCCESSFUL;
	}
...
static NTSTATUS ldapsam_delete_dom_group(struct pdb_methods *my_methods, TALLOC_CTX *tmp_ctx, uint32 rid)
{
	...
	if (num_result > 1) {
		DEBUG(1, ("ldapsam_delete_dom_group: More than one group with the same SID ?!\n"));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	dn = smbldap_talloc_dn(tmp_ctx, priv2ld(ldap_state), entry);
	if (!dn) {
		DEBUG(1,("ldapsam_delete_dom_group: Out of memory!\n"));
		return NT_STATUS_NO_MEMORY;
	}

	gidstr = smbldap_talloc_single_attribute(priv2ld(ldap_state), entry, "gidNumber", tmp_ctx);
	if (!gidstr) {
		DEBUG(1, ("ldapsam_delete_dom_group: Unable to find the group's gid!\n"));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
					...
					int modop)
{
	...
	if (num_result > 1) {
		DEBUG(1, ("ldapsam_change_groupmem: More than one account with the same SID ?!\n"));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	if (modop == LDAP_MOD_DELETE) {
		...
		gidstr = smbldap_talloc_single_attribute(priv2ld(ldap_state), entry, "gidNumber", tmp_ctx);
		if (!gidstr) {
			DEBUG(1, ("ldapsam_change_groupmem: Unable to find the member's gid!\n"));
			return NT_STATUS_INTERNAL_DB_CORRUPTION;
		}
		...
		if (!sid_to_gid(&group_sid, &group_gid)) {
			DEBUG(1, ("ldapsam_change_groupmem: Unable to get group gid from SID!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
	...
	if (!uidstr) {
		DEBUG(1, ("ldapsam_change_groupmem: Unable to find the member's name!\n"));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	if (num_result > 1) {
		DEBUG(1, ("ldapsam_change_groupmem: More than one group with the same SID ?!\n"));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	dn = smbldap_talloc_dn(tmp_ctx, priv2ld(ldap_state), entry);
	if (!dn) {
		DEBUG(1,("ldapsam_change_groupmem: Out of memory!\n"));
		return NT_STATUS_NO_MEMORY;
	}
					  ...
					  struct samu *sampass)
{
	...
	DEBUG(1,("ldapsam_set_primary_group: Attempt to set primary group for user [%s]\n", pdb_get_username(sampass)));

	if (!sid_to_gid(pdb_get_group_sid(sampass), &gid)) {
		DEBUG(1,("ldapsam_set_primary_group: failed to retrieve gid from user's group SID!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	gidstr = talloc_asprintf(mem_ctx, "%d", gid);
	if (!gidstr) {
		DEBUG(1,("ldapsam_set_primary_group: Out of Memory!\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG(1,("ldapsam_set_primary_group: user search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(mem_ctx, result);
	...
	if (num_result == 0) {
		DEBUG(1,("ldapsam_set_primary_group: user not found!\n"));
		return NT_STATUS_NO_SUCH_USER;
	}

	if (num_result > 1) {
		DEBUG(1, ("ldapsam_set_primary_group: More than one user with name [%s] ?!\n", pdb_get_username(sampass)));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG(1,("ldapsam_set_primary_group: failed to modify [%s] primary group to [%s]\n",
				      ...
				      const char *domain)
{
	...
	trusted_dn = smbldap_talloc_dn(talloc_tos(), priv2ld(ldap_state),
				       entry);
	if (trusted_dn == NULL) {
		DEBUG(1,("ldapsam_del_trusteddom_pw: Out of memory!\n"));
		return False;
	}
...
static NTSTATUS pdb_init_ldapsam_common(struct pdb_methods **pdb_method, const char *location)
{
	...
	if ( !(ldap_state = TALLOC_ZERO_P(*pdb_method, struct ldapsam_privates)) ) {
		DEBUG(1, ("pdb_init_ldapsam_common: talloc() failed for ldapsam private_data!\n"));
		return NT_STATUS_NO_MEMORY;
	}
...
NTSTATUS pdb_init_ldapsam(struct pdb_methods **pdb_method, const char *location)
{
	...
	entry = ldap_first_entry(ldap_state->smbldap_state->ldap_struct,
				 result);
	if (!entry) {
		DEBUG(1, ("pdb_init_ldapsam: Could not get domain info "
			  "entry\n"));
		ldap_msgfree(result);
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	if (alg_rid_base_string) {
		...
		if (alg_rid_base != algorithmic_rid_base()) {
			DEBUG(1, ("The value of 'algorithmic RID base' has "
