						 ...
						 uint32 *channel)
{
	...
	if (size != sizeof(*pass)) {
		DEBUG(1, ("secrets were of incorrect size!\n"));
		SAFE_FREE(pass);
		return False;
	}
                                           ...
                                           const DOM_SID *sid)
{
	...
	if (!push_ucs2_allocate(&uni_dom_name, domain, &converted_size)) {
		DEBUG(1, ("Could not convert domain name %s to unicode\n",
			  domain));
		return False;
	}
...
bool secrets_store_ldap_pw(const char* dn, char* pw)
{
	...
	if (asprintf(&key, "%s/%s", SECRETS_LDAP_BIND_PW, dn) < 0) {
		DEBUG(1, ("secrets_store_ldap_pw: asprintf failed!\n"));
		return False;
	}
...
bool fetch_ldap_pw(char **dn, char** pw)
{
	...
	*dn = smb_xstrdup(lp_ldap_admin_dn());
	...
	if (asprintf(&key, "%s/%s", SECRETS_LDAP_BIND_PW, *dn) < 0) {
		SAFE_FREE(*dn);
		DEBUG(1, ("fetch_ldap_pw: asprintf failed!\n"));
	}
	...
	*pw=(char *)secrets_fetch(key, &size);
	...
	if (!size) {
		...
		if (!old_style_key) {
			DEBUG(1, ("fetch_ldap_pw: strdup failed!\n"));
			return False;
		}
		...
		if (!secrets_store_ldap_pw(*dn, old_style_pw)) {
			DEBUG(1,("fetch_ldap_pw: ldap secret could not be upgraded!\n"));
			SAFE_FREE(old_style_key);
			SAFE_FREE(*dn);
			return False;
		}
		if (!secrets_delete(old_style_key)) {
			DEBUG(1,("fetch_ldap_pw: old ldap secret could not be deleted!\n"));
		}
...
static int list_trusted_domain(struct db_record *rec, void *private_data)
{
	...
	if (pass.domain_sid.num_auths != 4) {
		DEBUG(1, ("SID %s is not a domain sid, has %d "
			  "auths instead of 4\n",
...
static TDB_CONTEXT *open_schannel_session_store(TALLOC_CTX *mem_ctx)
{
	...
	if (!fname) {
        ...
        if (!tdb_sc) {
                DEBUG(1,("open_schannel_session_store: Failed to open %s\n", fname));
		TALLOC_FREE(fname);
                return NULL;
        }
	...
	if (vers.dptr == NULL) {
		...
		if (ver != 1) {
			tdb_close(tdb_sc);
			tdb_sc = NULL;
			DEBUG(1,("open_schannel_session_store: wrong version number %d in %s\n",
				(int)ver, fname ));
		}
	} else {
		tdb_close(tdb_sc);
		tdb_sc = NULL;
		DEBUG(1,("open_schannel_session_store: wrong version number size %d in %s\n",
				...
				struct dcinfo **ppdc)
{
	...
	*ppdc = NULL;
	...
	if (!value.dptr) {
		DEBUG(1,("secrets_restore_schannel_session_info: Failed to find entry with key %s\n",
			keystr ));
		tdb_close(tdb_sc);
		return False;
	}
