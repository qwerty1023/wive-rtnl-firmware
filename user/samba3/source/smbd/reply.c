			...
			switch(siz) {
				...
				case 1:
					*d++ = *s++;
					break;
				default:
					DEBUG(1,("check_path_syntax_internal: character length assumptions invalid !\n"));
					*d = '\0';
	...
	*name1 = *name2 = 0;
	...
	case 0x81: /* session request */
		...
		if (name_len(inbuf+4) > 50 || 
			...
			DEBUG(1,("Invalid name length in session request\n"));
			return;
		}
	...
	case 0x84: /* retarget session response */
		DEBUG(1,("Unexpected session response\n"));
		break;
...
{
	DEBUG(1, ("unknown command type (%s): type=%d (0x%X)\n",
...
void reply_mknew(struct smb_request *req)
{
	...
	if (fattr & aVOLID) {
		DEBUG(1,("Attempt to create file (%s) with volid set - "
			"please report this\n", fname));
	}
	...
	if ( (chain_size == 0) && (nread > 0) && (fsp->base_fsp == NULL) &&
		...
		if (SMB_VFS_SENDFILE(smbd_server_fd(), fsp,
			...
			if (errno == EINTR) {
				...
				DEBUG(1,("send_file_readbraw: sendfile not available. Faking..\n"));

				if (fake_sendfile(fsp, startpos, nread) == -1) {
					DEBUG(1,("send_file_readbraw: fake_sendfile failed for file %s (%s).\n",
			...
			DEBUG(1,("send_file_readbraw: sendfile failed for file %s (%s). Terminating\n",
				fsp->fsp_name, strerror(errno) ));
...
normal_readbraw:
	...
	if (!outbuf) {
		DEBUG(1,("send_file_readbraw: TALLOC_ARRAY failed for size %u.\n",
			(unsigned)(nread+4)));
...
void reply_readbraw(struct smb_request *req)
{
	...
	if(req->wct == 10) {
		...
		if(IVAL(req->inbuf,smb_vwv8) != 0) {
			DEBUG(1,("reply_readbraw: large offset "
		...
		if(startpos < 0) {
			DEBUG(1,("reply_readbraw: negative 64 bit "
				"readraw offset (%.0f) !\n",
				(double)startpos ));
			reply_readbraw_error();
			END_PROFILE(SMBreadbraw);
...
void reply_lockread(struct smb_request *req)
{
	...
	if (numtoread > max_recv) {
		DEBUG(1,("reply_lockread: requested read size (%u) is greater than maximum allowed (%u). \
...
void reply_read(struct smb_request *req)
{
	 ...
	 */
	if (numtoread > max_recv) {
		DEBUG(1,("reply_read: requested read size (%u) is greater than maximum allowed (%u). \
Returning short read of maximum allowed for compatibility with Windows 2000.\n",
			(unsigned int)numtoread, (unsigned int)max_recv ));
		numtoread = MIN(numtoread,max_recv);
	}
	...
	if ((chain_size == 0) && (CVAL(req->inbuf,smb_vwv0) == 0xFF) &&
		...
		if ((nread = SMB_VFS_SENDFILE(smbd_server_fd(), fsp, &header, startpos, smb_maxcnt)) == -1) {
			...
			if (errno == EINTR) {
				/* Ensure we don't do this again. */
				set_use_sendfile(SNUM(conn), False);
				DEBUG(1,("send_file_readX: sendfile not available. Faking..\n"));
				nread = fake_sendfile(fsp, startpos,
						      smb_maxcnt);
				if (nread == -1) {
					DEBUG(1,("send_file_readX: fake_sendfile failed for file %s (%s).\n",
						fsp->fsp_name, strerror(errno) ));
			...
			DEBUG(1,("send_file_readX: sendfile failed for file %s (%s). Terminating\n",
				fsp->fsp_name, strerror(errno) ));
...
normal_read:
	...
	if ((smb_maxcnt & 0xFF0000) > 0x10000) {
		...
		if (write_data(smbd_server_fd(), (char *)headerbuf,
			...
			DEBUG(1,("send_file_readX: write_data failed for file %s (%s). Terminating\n",
				fsp->fsp_name, strerror(errno) ));
			exit_server_cleanly("send_file_readX sendfile failed");
		}
...
void reply_read_and_X(struct smb_request *req)
{
	...
	if (req->wct == 12) {
		...
		if(IVAL(req->inbuf,smb_vwv10) != 0) {
			DEBUG(1,("reply_read_and_X - large offset (%x << 32) "
...
void reply_writebraw(struct smb_request *req)
{
	...
	if (numtowrite != 0) {
		...
		if (numtowrite > 0xFFFF) {
			DEBUG(1,("reply_writebraw: Oversize secondary write "
...
void reply_write_and_X(struct smb_request *req)
{
	...
	if(req->wct == 14) {
		...
		if(IVAL(req->inbuf,smb_vwv12) != 0) {
			DEBUG(1,("reply_write_and_X - large offset (%x << 32) "
				 "used and we don't support 64 bit offsets.\n",
...
void reply_echo(struct smb_request *req)
{
	...
	if (data_len > BUFFER_SIZE) {
		DEBUG(1,("reply_echo: data_len too large.\n"));
	...
	if (smb_reverb > 100) {
		DEBUG(1,("large reverb (%d)?? Setting to 100\n",smb_reverb));
		smb_reverb = 100;
	}
			...
			bool target_is_directory)
{
	...
	if ((ofun&3) == 1) {
		if(SMB_VFS_LSEEK(fsp2,0,SEEK_END) == -1) {
			DEBUG(1,("copy_file: error - vfs lseek returned error %s\n", strerror(errno) ));
			/*
...
void reply_lockingX(struct smb_request *req)
{
	...
	*/
	if ((locktype & LOCKING_ANDX_OPLOCK_RELEASE)) {
		...
		if (!result) {
			DEBUG(1, ("reply_lockingX: error in removing "
				  "oplock on file %s\n", fsp->fsp_name));
		...
		if (num_locks == 0 && num_ulocks == 0) {
			/* Sanity check - ensure a pure oplock break is not a
			   chained request. */
			if(CVAL(req->inbuf,smb_vwv0) != 0xff)
				DEBUG(1,("reply_lockingX: Error : pure oplock "
