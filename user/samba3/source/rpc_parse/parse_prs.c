		     ...
		     int from_off, int to_off)
{
	...
	if (fd != -1) {
		...
		if ( (sz != to_off-from_off) || (i != 0) ) {
			DEBUG(1,("Error writing/closing %s: %ld!=%ld %d\n", fname, (unsigned long)sz, (unsigned long)to_off-from_off, i ));
		} else {
			DEBUG(1,("created %s\n", fname));
		}
	}
...
bool prs_init(prs_struct *ps, uint32 size, TALLOC_CTX *ctx, bool io)
{
	...
	if (size != 0) {
		...
		if((ps->data_p = (char *)SMB_MALLOC((size_t)size)) == NULL) {
			DEBUG(1,("prs_init: malloc fail for %u bytes.\n", (unsigned int)size));
			return False;
		}
...
bool prs_set_buffer_size(prs_struct *ps, uint32 newsize)
{
	...
	if (newsize < ps->buffer_size) {
		...
		if (newsize == 0) {
			...
			if (ps->data_p == NULL) {
				DEBUG(1,("prs_set_buffer_size: Realloc failure for size %u.\n",
					(unsigned int)newsize));
				DEBUG(1,("prs_set_buffer_size: Reason %s\n",strerror(errno)));
				return False;
			}
		}
	}
...
bool prs_grow(prs_struct *ps, uint32 extra_space)
{
	...
	if(UNMARSHALLING(ps) || !ps->is_dynamic) {
		DEBUG(1,("prs_grow: Buffer overflow - unable to expand buffer by %u bytes.\n",
	...
	if(ps->buffer_size == 0) {
		...
		if((ps->data_p = (char *)SMB_MALLOC(new_size)) == NULL) {
			DEBUG(1,("prs_grow: Malloc failure for size %u.\n", (unsigned int)new_size));
			return False;
		}
		...
		if ((ps->data_p = (char *)SMB_REALLOC(ps->data_p, new_size)) == NULL) {
			DEBUG(1,("prs_grow: Realloc failure for size %u.\n",
				(unsigned int)new_size));
			return False;
		}
...
bool prs_force_grow(prs_struct *ps, uint32 extra_space)
{
	...
	if(!UNMARSHALLING(ps) || !ps->is_dynamic) {
		DEBUG(1,("prs_force_grow: Buffer overflow - unable to expand buffer by %u bytes.\n",
				(unsigned int)extra_space));
		return False;
	}

	if((ps->data_p = (char *)SMB_REALLOC(ps->data_p, new_size)) == NULL) {
		DEBUG(1,("prs_force_grow: Realloc failure for size %u.\n",
			(unsigned int)new_size));
		return False;
	}
...
char *prs_mem_get(prs_struct *ps, uint32 extra_size)
{
	if(UNMARSHALLING(ps)) {
		...
		if (ps->data_offset + extra_size > ps->buffer_size) {
			DEBUG(1,("prs_mem_get: reading data of size %u would overrun "
				"buffer by %u bytes.\n",
