...
static char *string_create_s(int size)
{
	...
	if (tmp == NULL) {
		DEBUG(1, ("Out of memory in string_create_s\n"));
	}
	...
	if ((hb->dbuf.linkflag != '0') && (hb -> dbuf.linkflag != '5')) {
		if (hb->dbuf.linkflag == 0) {
			...
			if (hb -> dbuf.linkflag == 'L') { /* We have a longlink */
				...
				DEBUG(1, ("this tar file appears to contain some kind \
...
static void dozerobuf(int f, int n)
{
	...
	if (n+tp >= tbufsiz) {
		...
		if (sys_write(f, tarbuf, tbufsiz) != tbufsiz) {
			DEBUG(1, ("dozerobuf: sys_write fail\n"));
			return;
		}
...
static void dotareof(int f)
{
	...
	if (sys_fstat(f, &stbuf) == -1) {
		DEBUG(1, ("Couldn't stat file handle\n"));
		return;
	}
	...
	if (tp > 0) {
		...
		if (sys_write(f, tarbuf, towrite) != towrite) {
			DEBUG(1,("dotareof: sys_write fail\n"));
		}
	}
}
...
static bool ensurepath(const char *fname)
{
	...
	if ((partpath == NULL) || (ffname == NULL)){
		DEBUG(1, ("Out of memory in ensurepath: %s\n", fname));
		SAFE_FREE(partpath);
		SAFE_FREE(ffname);
		return(False);
	}
...
static void do_atar(const char *rname_in,char *lname,file_info *finfo1)
{
	...
	if (fnum == -1) {
		DEBUG(1,("%s opening remote file %s (%s)\n",
	...
	if (finfo.name == NULL) {
		DEBUG(1, ("Unable to allocate space for finfo.name in do_atar\n"));
		goto cleanup;
	}
	...
	if (tar_inc && !(finfo.mode & aARCH)) {
		...
		do {
			...
			if (datalen == -1) {
				DEBUG(1,("Error reading file %s : %s\n", rname, cli_errstr(cli)));
				break;
			}
			...
			*/
			...
			if (nread > finfo.size) {
				...
				DEBUG(1,("File size change - truncating %s to %.0f bytes\n",
			...
			* write out in 512 byte intervals */
			...
			if (dotarbuf(tarhandle,data,datalen) != datalen) {
				DEBUG(1,("Error writing to tar file - %s\n", strerror(errno)));
				break;
			}

			if ( (datalen == 0) && (finfo.size != 0) ) {
				DEBUG(1,("Error reading file %s. Got 0 bytes\n", rname));
				break;
			}
		...
		if (wrote_tar_header) {
			...
			if (nread < finfo.size) {
				DEBUG(1, ("Didn't get entire file. size=%.0f, nread=%d\n",
							(double)finfo.size, (int)nread));
				if (padit(data, (SMB_BIG_UINT)sizeof(data), finfo.size - nread))
					DEBUG(1,("Error writing tar file - %s\n", strerror(errno)));
			}
...
static int skip_file(int skipsize)
{
	...
	while (dsize > 0) {
		if (next_block(tarbuf, &buffer_p, tbufsiz) <= 0) {
			DEBUG(1, ("Empty file, short tar file, or read error: %s\n", strerror(errno)));
			return(False);
		}
		dsize -= TBLOCK;
	}
...
static int get_file(file_info2 finfo)
{
	...
	while (rsize > 0) {
		...
		if (cli_write(cli, fnum, 0, buffer_p + bpos, pos, dsize) != dsize) {
			DEBUG(1, ("Error writing remote file\n"));
			return 0;
		}
	...
	if (!cli_close(cli, fnum)) {
		DEBUG(1, ("Error closing remote file\n"));
		return(False);
	}
	...
	if (!cli_setatr(cli, finfo.name, finfo.mode, finfo.mtime_ts.tv_sec)) {
		if (tar_real_noisy) {
			DEBUG(1, ("Could not set time on file: %s\n", finfo.name));
			/*return(False); */ /* Ignore, as Win95 does not allow changes */
		}
	}

	ntarf++;
	DEBUG(1, ("restore tar file %s of size %.0f bytes\n", finfo.name, (double)finfo.size));
	return(True);
}
...
{
	DEBUG(1, ("restore directory %s\n", finfo.name));

	if (!ensurepath(finfo.name)) {
		DEBUG(1, ("Problems creating directory\n"));
		return(False);
	}
	ntarf++;
	return(True);
...
static char *get_longfilename(file_info2 finfo)
{
	...
	if (longname == NULL) {
		DEBUG(1, ("could not allocate buffer of size %d for longname\n", namesize));
		return(NULL);
	}
		...
		switch (readtarheader((union hblock *) buffer_p,
			...
			case -2:    /* Hmm, not good, but not fatal */
				...
				if ((next_block(tarbuf, &buffer_p, tbufsiz) <= 0) && !skip_file(finfo.size)) {
					DEBUG(1, ("Short file, bailing out...\n"));
					return;
				}
				break;

			case -1:
				DEBUG(1, ("abandoning restore, -1 from read tar header\n"));
				return;

			case 0: /* chksum is zero - looks like an EOF */
				DEBUG(1, ("tar: restored %d files and directories\n", ntarf));
...
int cmd_block(void)
{
	...
	if (block < 0 || block > 65535) {
		DEBUG(1, ("blocksize out of range"));
		return 1;
	}
...
int cmd_tarmode(void)
{
	...
	while (next_token_talloc(ctx, &cmd_ptr,&buf,NULL)) {
		if (strequal(buf, "full"))
			...
			DEBUG(1, ("tarmode: unrecognised option %s\n", buf));
		TALLOC_FREE(buf);
	}

	DEBUG(1, ("tarmode is now %s, %s, %s, %s, %s\n",
		...
		while(*q) {
			switch (*q++) {
				...
				case 'a':
					...
					break;
				default:
					DEBUG(1, ("setmode <filename> <perm=[+|-]rsha>\n"));
					return 1;
			}
		}
	}
...
int cmd_tar(void)
{
	...
	if (!next_token_talloc(ctx, &cmd_ptr,&buf,NULL)) {
		DEBUG(1,("tar <c|x>[IXbgan] <filename>\n"));
		return 1;
	}
...
int process_tar(void)
{
	...
	switch(tar_type) {
		...
		case 'c':
			...
			DEBUG(1, ("tar: dumped %d files and directories\n", ntarf));
			DEBUG(1, ("Total bytes written: %.0f\n", (double)ttarf));
			break;
	}
...
static int read_inclusion_file(char *filename)
{
	...
	if ((inclusion = x_fopen(filename, O_RDONLY, 0)) == NULL) {
		 ...
		 */
		DEBUG(1,("Unable to open inclusion file %s\n", filename));
		return 0;
	}
	...
	while ((! error) && (x_fgets(buf, sizeof(buf)-1, inclusion))) {
		if (inclusion_buffer == NULL) {
			...
			if ((inclusion_buffer = (char *)SMB_MALLOC(inclusion_buffer_size)) == NULL) {
				DEBUG(1,("failure allocating buffer to read inclusion file\n"));
				error = 1;
				break;
			}
		}
		...
		if ((strlen(buf) + 1 + inclusion_buffer_sofar) >= inclusion_buffer_size) {
			...
			if (!inclusion_buffer) {
				DEBUG(1,("failure enlarging inclusion buffer to %d bytes\n",
	...
	if (! error) {
		...
		if (cliplist == NULL) {
			DEBUG(1,("failure allocating memory for cliplist\n"));
			error = 1;
		} else {
			cliplist[clipn] = NULL;
			p = inclusion_buffer;
			for (i = 0; (! error) && (i < clipn); i++) {
				...
				if ((tmpstr = (char *)SMB_MALLOC(strlen(p)+1)) == NULL) {
					DEBUG(1, ("Could not allocate space for a cliplist item, # %i\n", i));
					error = 1;
				} else {
					unfixtarname(tmpstr, p, strlen(p) + 1, True);
					cliplist[i] = tmpstr;
					if ((p = strchr_m(p, '\000')) == NULL) {
						DEBUG(1,("INTERNAL ERROR: inclusion_buffer is of unexpected contents.\n"));
						abort();
					}
				}
				++p;
...
int tar_parseargs(int argc, char *argv[], const char *Optarg, int Optind)
{
		...
		switch(*Optarg++) {
			...
			case 'b':
				if (Optind>=argc || !(blocksize=atoi(argv[Optind]))) {
					DEBUG(1,("Option b must be followed by valid blocksize\n"));
					return 0;
				} else {
					Optind++;
					newOptind++;
			...
			case 'N':
				if (Optind>=argc) {
					DEBUG(1,("Option N must be followed by valid file name\n"));
					return 0;
				} else {
					SMB_STRUCT_STAT stbuf;
					...
					if (sys_stat(argv[Optind], &stbuf) == 0) {
						...
						DEBUG(1,("Error setting newer-than time\n"));
						return 0;
					}
				}
				break;
			...
			case 'I':
				if (tar_clipfl) {
					DEBUG(1,("Only one of I,X,F must be specified\n"));
					return 0;
				}
				tar_clipfl='I';
				break;
			...
			case 'n':
				if (tar_type == 'c') {
					...
					DEBUG(1, ("n is only meaningful when creating a tar-file\n"));
					return 0;
				}
				break;
			default:
				DEBUG(1,("Unknown tar option\n"));
				return 0;
		}
	}
	...
	if (tar_clipfl=='F') {
		if (argc-Optind-1 != 1) {
			DEBUG(1,("Option F must be followed by exactly one filename.\n"));
			return 0;
		}
		newOptind++;
		...
		if ((tmplist=SMB_MALLOC_ARRAY(char *,clipn)) == NULL) {
			DEBUG(1, ("Could not allocate space to process cliplist, count = %i\n", clipn));
			return 0;
		}
	...
	if (Optind>=argc || !strcmp(argv[Optind], "-")) {
		...
		if (!argv[Optind]) {
			DEBUG(1,("Must specify tar filename\n"));
			return 0;
		}
		...
		if (tar_type=='c' && dry_run) {
			...
			DEBUG(1,("Error opening local file %s - %s\n", argv[Optind], strerror(errno)));
			return(0);
		}
		newOptind++;
	}
