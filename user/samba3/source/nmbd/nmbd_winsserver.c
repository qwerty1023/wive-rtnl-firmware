...
bool initialise_wins(void)
{
	...
	if (!wins_tdb) {
		DEBUG(1,("initialise_wins: failed to open wins.tdb. Error was %s\n",
			strerror(errno) ));
		return False;
	}
	...
	while (!x_feof(fp)) {
		...
		if (strncmp(line,"VERSION ", 8) == 0) {
			if (sscanf(line,"VERSION %d %u", &version, &hash) != 2 ||
				...
				DEBUG(1,("Discarding invalid wins.dat file [%s]\n",line));
				x_fclose(fp);
				return True;
			}
			continue;
		...
		if (!next_token_talloc(frame,&ptr,&name_str,NULL)) {
			DEBUG(1,("initialise_wins: Failed to parse name when parsing line %s\n", line ));
			TALLOC_FREE(frame);
			continue;
		}

		if (!next_token_talloc(frame,&ptr,&ttl_str,NULL)) {
			DEBUG(1,("initialise_wins: Failed to parse time to live when parsing line %s\n", line ));
			TALLOC_FREE(frame);
			continue;
		}
		...
		if(num_ips == 0) {
			DEBUG(1,("initialise_wins: Missing IP address when parsing line %s\n", line ));
			TALLOC_FREE(frame);
			continue;
		}

		if(!got_token) {
			DEBUG(1,("initialise_wins: Missing nb_flags when parsing line %s\n", line ));
			TALLOC_FREE(frame);
			continue;
		}
		...
		if((ip_list = SMB_MALLOC_ARRAY( struct in_addr, num_ips)) == NULL) {
			DEBUG(1,("initialise_wins: Malloc fail !\n"));
			x_fclose(fp);
			TALLOC_FREE(frame);
			return False;
		}
                                            ...
                                            struct packet_struct *p)
{
	...
	if(bcast) {
		...
		DEBUG(1,("wins_process_name_registration_request: broadcast name registration request \
                                                        ...
                                                        struct packet_struct *p)
{
	...
	if(bcast) {
		...
		DEBUG(1,("wins_process_multihomed_name_registration_request: broadcast name registration request \
received for name %s from IP %s on subnet %s. Error - should not be sent to WINS server\n",
			nmb_namestr(question), inet_ntoa(from_ip), subrec->subnet_name));
		return;
	}
	...
	if(group) {
		DEBUG(1,("wins_process_multihomed_name_registration_request: group name registration request \
...
{  
	...
	if((prdata = (char *)SMB_MALLOC( num_ips * 6 )) == NULL) {
		DEBUG(1,("process_wins_dmb_query_request: Malloc fail !.\n"));
		return;
	}
	...
	if(rcode == 0) {
		...
		if( namerec->data.num_ips == 1 ) {
			...
			if((prdata = (char *)SMB_MALLOC( namerec->data.num_ips * 6 )) == NULL) {
				DEBUG(1,("send_wins_name_query_response: malloc fail !\n"));
				return;
			}
		}
                                       ...
                                       struct packet_struct *p)
{
	...
	if(bcast) {
		...
		DEBUG(1,("wins_process_name_release_request: broadcast name registration request \
received for name %s from IP %s on subnet %s. Error - should not be sent to WINS server\n",
			nmb_namestr(question), inet_ntoa(from_ip), subrec->subnet_name));
		return;
	}
		...
		if (ip_equal_v4(namerec->data.wins_ip, our_fake_ip)) {
			...
			switch (namerec->data.wins_flags & WINS_STATE_MASK) {
				...
				case WINS_TOMBSTONED:
					...
					goto done;
				case WINS_RELEASED:
					DEBUG(1,("wins_processing_traverse_fn: %s is in released state and\
...
void wins_write_database(time_t t, bool background)
{
	...
	if (background) {
		...
		if (tdb_reopen(wins_tdb)) {
			DEBUG(1,("wins_write_database: tdb_reopen failed. Error was %s\n",
				strerror(errno)));
			_exit(0);
			return;
		}
	...
	if((fp = x_fopen(fnamenew,O_WRONLY|O_CREAT,0644)) == NULL) {
		DEBUG(1,("wins_write_database: Can't open %s. Error was %s\n", fnamenew, strerror(errno)));
		goto err_exit;
	}
