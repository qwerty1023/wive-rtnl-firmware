				   ...
				   struct netr_DELTA_USER *r)
{
	...
	if (!pdb_getgrsid(&map, group_sid)) {
		DEBUG(1, ("Primary group of %s has no mapping!\n",
			  pdb_get_username(sam_account)));
	} else {
		if (map.gid != passwd->pw_gid) {
			if (!(grp = getgrgid(map.gid))) {
				DEBUG(1, ("Could not find unix group %lu for user %s (group SID=%s)\n",
				     ...
				     struct netr_DELTA_GROUP_MEMBER *r)
{
	...
	if (!get_domain_group_from_sid(group_sid, &map)) {
		DEBUG(1, ("Could not find global group %d\n", rid));
		return NT_STATUS_NO_SUCH_GROUP;
	}

	if (!(grp = getgrgid(map.gid))) {
		DEBUG(1, ("Could not find unix group %lu\n", (unsigned long)map.gid));
		return NT_STATUS_NO_SUCH_GROUP;
	}
