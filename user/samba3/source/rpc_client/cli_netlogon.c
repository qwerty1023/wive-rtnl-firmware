				     ...
				     uint32_t *neg_flags_inout)
{
 ...
 again:
	...
	if (!netlogon_creds_client_check(dc, &srv_chal_recv)) {
		 ...
		 */
		DEBUG(1,("rpccli_netlogon_setup_creds: server %s "
				   ...
				   int logon_type)
{
	...
	default:
		DEBUG(1, ("switch value %d not supported\n",
			logon_type));
	...
	if (memcmp(zeros, &ret_creds.cred.data, sizeof(ret_creds.cred.data)) != 0) {
		...
		if (!netlogon_creds_client_check(cli->dc, &ret_creds.cred)) {
			DEBUG(1,("rpccli_netlogon_sam_logon: credentials chain check failed\n"));
			return NT_STATUS_ACCESS_DENIED;
		}
	}
					   ...
					   struct netr_SamInfo3 **info3)
{
	...
	*info3 = NULL;
	...
	if (memcmp(zeros, ret_creds.cred.data, sizeof(ret_creds.cred.data)) != 0) {
		/* Check returned credentials if present. */
		if (!netlogon_creds_client_check(cli->dc, &ret_creds.cred)) {
			DEBUG(1,("rpccli_netlogon_sam_network_logon: credentials chain check failed\n"));
			return NT_STATUS_ACCESS_DENIED;
		}
	}
					    ...
					    uint32_t sec_channel_type)
{
	...
	if (neg_flags & NETLOGON_NEG_PASSWORD_SET2) {
		...
		result = rpccli_netr_ServerPasswordSet2(cli, mem_ctx,
							...
							&new_password);
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG(1,("rpccli_netr_ServerPasswordSet2 failed: %s\n",
				nt_errstr(result)));
			return result;
		}
	} else {
		...
		result = rpccli_netr_ServerPasswordSet(cli, mem_ctx,
						       ...
						       &new_password);
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG(1,("rpccli_netr_ServerPasswordSet failed: %s\n",
				nt_errstr(result)));
			return result;
		}
	}
	...
	if (!netlogon_creds_client_check(cli->dc, &srv_cred.cred)) {
		DEBUG(1,("credentials chain check failed\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
