...
static int map_pipe_auth_type_to_rpc_auth_type(enum pipe_auth_type auth_type)
{
	...
	default:
		DEBUG(1,("map_pipe_auth_type_to_rpc_type: unknown pipe "
			"auth type %u\n",
	...
	*/
       if (cli_is_dos_error(cli)) {
	       ...
	       if (eclass != ERRDOS && ecode != ERRmoredata) {
		       DEBUG(1,("rpc_read: DOS Error %d/%u (%s) in cli_read "
	...
	*/
       if (cli_is_nt_error(cli)) {
	       if (!NT_STATUS_EQUAL(cli_nt_error(cli),
		       ...
		       DEBUG(1,("rpc_read: Error (%s) in cli_read on fnum "
       ...
       if (num_read == -1) {
	       DEBUG(1,("rpc_read: Error - cli_read on fnum 0x%x returned "
			"-1\n", fnum));
			...
			uint32 *current_pdu_offset)
{
	...
	if (extra_data_size > 0) {
		if(!prs_force_grow(current_pdu, (uint32)extra_data_size)) {
			DEBUG(1,("rpc_read: Failed to grow parse struct by %d bytes.\n", (int)extra_data_size ));
	...
	do {
		...
		case NCACN_UNIX_STREAM:
			...
			break;
		default:
			DEBUG(1, ("unknown transport type %d\n",
				  cli->transport_type));
...
static NTSTATUS cli_pipe_get_current_pdu(struct rpc_pipe_client *cli, RPC_HDR *prhdr, prs_struct *current_pdu)
{
	...
	if(!smb_io_rpc_hdr("rpc_hdr   ", prhdr, current_pdu, 0)) {
		DEBUG(1,("cli_pipe_get_current_pdu: Failed to unmarshall RPC_HDR.\n"));
				...
				uint8 *p_ss_padding_len)
{
	...
	if ((auth_len > RPC_MAX_SIGN_SIZE) ||
		...
		DEBUG(1,("cli_pipe_verify_ntlmssp: auth_len %u is too large.\n",
	...
	if(!prs_set_offset(current_pdu, RPC_HEADER_LEN + RPC_HDR_RESP_LEN + data_len)) {
		DEBUG(1,("cli_pipe_verify_ntlmssp: cannot move offset to %u.\n",
	...
	if(!smb_io_rpc_hdr_auth("hdr_auth", &auth_info, current_pdu, 0)) {
		DEBUG(1,("cli_pipe_verify_ntlmssp: failed to unmarshall RPC_HDR_AUTH.\n"));
		return NT_STATUS_BUFFER_TOO_SMALL;
	}
	...
	switch (cli->auth->auth_level) {
		case PIPE_AUTH_LEVEL_PRIVACY:
			...
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG(1,("cli_pipe_verify_ntlmssp: failed to unseal "
		...
		case PIPE_AUTH_LEVEL_INTEGRITY:
			...
			status = ntlmssp_check_packet(ntlmssp_state,
							...
							&auth_blob);
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG(1,("cli_pipe_verify_ntlmssp: check signing failed on "
					"packet from %s. Error was %s.\n",
					rpccli_pipe_txt(debug_ctx(), cli),
					nt_errstr(status) ));
				return status;
			...
			break;
		default:
			DEBUG(1, ("cli_pipe_verify_ntlmssp: unknown internal "
				  "auth level %d\n", cli->auth->auth_level));
	...
	if(!prs_set_offset(current_pdu, save_offset)) {
		DEBUG(1,("api_pipe_auth_process: failed to set offset back to %u\n",
				...
				uint8 *p_ss_padding_len)
{
	...
	if (auth_len < RPC_AUTH_SCHANNEL_SIGN_OR_SEAL_CHK_LEN) {
		DEBUG(1,("cli_pipe_verify_schannel: auth_len %u.\n", (unsigned int)auth_len ));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if ((auth_len > RPC_MAX_SIGN_SIZE) ||
			(RPC_HEADER_LEN + RPC_HDR_RESP_LEN + RPC_HDR_AUTH_LEN + auth_len > prhdr->frag_len)) {
		DEBUG(1,("cli_pipe_verify_schannel: auth_len %u is too large.\n",
			(unsigned int)auth_len ));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if(!prs_set_offset(current_pdu, RPC_HEADER_LEN + RPC_HDR_RESP_LEN + data_len)) {
		DEBUG(1,("cli_pipe_verify_schannel: cannot move offset to %u.\n",
			(unsigned int)RPC_HEADER_LEN + RPC_HDR_RESP_LEN + data_len ));
		return NT_STATUS_BUFFER_TOO_SMALL;
	}

	if(!smb_io_rpc_hdr_auth("hdr_auth", &auth_info, current_pdu, 0)) {
		DEBUG(1,("cli_pipe_verify_schannel: failed to unmarshall RPC_HDR_AUTH.\n"));
		return NT_STATUS_BUFFER_TOO_SMALL;
	}

	if (auth_info.auth_type != RPC_SCHANNEL_AUTH_TYPE) {
		DEBUG(1,("cli_pipe_verify_schannel: Invalid auth info %d on schannel\n",
			auth_info.auth_type));
		return NT_STATUS_BUFFER_TOO_SMALL;
	}
	...
	if(!smb_io_rpc_auth_schannel_chk("", RPC_AUTH_SCHANNEL_SIGN_OR_SEAL_CHK_LEN,
		...
		DEBUG(1,("cli_pipe_verify_schannel: failed to unmarshal RPC_AUTH_SCHANNEL_CHK.\n"));
		return NT_STATUS_BUFFER_TOO_SMALL;
	}
			...
			prs_struct *return_data)
{
	...
	*pdata_len = current_pdu_len;
	...
	switch (prhdr->pkt_type) {
			...
			*pdata_len = current_pdu_len - RPC_HEADER_LEN - RPC_HDR_RESP_LEN - ss_padding_len;
			...
			if ((prs_data_size(return_data) == 0) && rhdr_resp.alloc_hint && (rhdr_resp.alloc_hint < 15*1024*1024)) {
				if (!prs_set_buffer_size(return_data, rhdr_resp.alloc_hint)) {
					DEBUG(1,("cli_pipe_validate_current_pdu: reply alloc hint %u "
		...
		default:
			DEBUG(1, ("cli_pipe_validate_current_pdu: unknown packet type %u received "
				"from %s!\n",
			...
			uint8 expected_pkt_type)
{
	...
	case NCACN_NP: {
		...
		{
			DEBUG(1, ("rpc_api_pipe: %s returned critical error. "
				  "Error was %s\n",
				  rpccli_pipe_txt(debug_ctx(), cli),
	...
	case NCACN_UNIX_STREAM:
	{
		...
		if (nwritten == -1) {
			...
			DEBUG(1, ("rpc_api_pipe: write_data returned %s\n",
				  strerror(errno)));
			goto err;
		}
		rparam = NULL;
		...
		if ((rhdr.flags & RPC_FLG_FIRST)) {
			if (rhdr.pack_type[0] == 0) {
				...
				if (current_pdu.bigendian_data != rbuf->bigendian_data) {
					DEBUG(1,("rpc_api_pipe: Error : Endianness changed from %s to %s\n",
						...
						prs_struct *auth_data)
{
	...
	if(!smb_io_rpc_auth_schannel_neg("schannel_neg",
		...
		DEBUG(1,("Failed to marshall RPC_AUTH_SCHANNEL_NEG.\n"));
		prs_mem_free(auth_data);
		return NT_STATUS_NO_MEMORY;
	}
						...
						prs_struct *pauth_info)
{
	...
	if(!smb_io_rpc_hdr("hdr"   , &hdr, rpc_out, 0)) {
		DEBUG(1,("create_bind_or_alt_ctx_internal: failed to marshall RPC_HDR.\n"));
		return NT_STATUS_NO_MEMORY;
	}

	/* Marshall the bind request data */
	if(!smb_io_rpc_hdr_rb("", &hdr_rb, rpc_out, 0)) {
		DEBUG(1,("create_bind_or_alt_ctx_internal: failed to marshall RPC_HDR_RB.\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	if(auth_len != 0) {
		if (ss_padding_len) {
			...
			if (!prs_copy_data_in(rpc_out, pad, ss_padding_len)) {
				DEBUG(1,("create_bind_or_alt_ctx_internal: failed to marshall padding.\n"));
				return NT_STATUS_NO_MEMORY;
			}
		}

		if(!smb_io_rpc_hdr_auth("hdr_auth", phdr_auth, rpc_out, 0)) {
			DEBUG(1,("create_bind_or_alt_ctx_internal: failed to marshall RPC_HDR_AUTH.\n"));
			return NT_STATUS_NO_MEMORY;
		}


		if(!prs_append_prs_data( rpc_out, pauth_info)) {
			DEBUG(1,("create_bind_or_alt_ctx_internal: failed to grow parse struct to add auth.\n"));
			return NT_STATUS_NO_MEMORY;
		}
	}
					...
					prs_struct *outgoing_pdu)
{
	...
	if(!smb_io_rpc_hdr_auth("hdr_auth", &auth_info, outgoing_pdu, 0)) {
		DEBUG(1,("add_ntlmssp_auth_footer: failed to marshall RPC_HDR_AUTH.\n"));
	...
	if (!prs_copy_data_in(outgoing_pdu, (const char *)auth_blob.data, NTLMSSP_SIG_SIZE)) {
		DEBUG(1,("add_ntlmssp_auth_footer: failed to add %u bytes auth blob.\n",
					...
					prs_struct *outgoing_pdu)
{
	...
	if(!smb_io_rpc_hdr_auth("hdr_auth", &auth_info, outgoing_pdu, 0)) {
		DEBUG(1,("add_schannel_auth_footer: failed to marshall RPC_HDR_AUTH.\n"));
		return NT_STATUS_NO_MEMORY;
	}
				...
				prs_struct *rpc_out)
{
	...
	/* Marshall it. */
	if(!smb_io_rpc_hdr("hdr", &hdr, rpc_out, 0)) {
		DEBUG(1,("create_rpc_bind_auth3: failed to marshall RPC_HDR.\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	*/
	...
	if (!prs_uint32("pad", rpc_out, 0, &pad)) {
		DEBUG(1,("create_rpc_bind_auth3: failed to marshall 4 byte pad.\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	if(!smb_io_rpc_hdr_auth("hdr_auth", &hdr_auth, rpc_out, 0)) {
		DEBUG(1,("create_rpc_bind_auth3: failed to marshall RPC_HDR_AUTHA.\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	if(!prs_copy_data_in(rpc_out, (char *)pauth_blob->data, pauth_blob->length)) {
		DEBUG(1,("create_rpc_bind_auth3: failed to marshall auth blob.\n"));
		return NT_STATUS_NO_MEMORY;
	}
				...
				enum pipe_auth_level auth_level)
{
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG(1,("rpc_finish_auth3_bind: NTLMSSP update using server blob failed.\n"));
	...
	if (ret != (ssize_t)prs_offset(&rpc_out)) {
		DEBUG(1,("rpc_send_auth_auth3: write failed. Return was %s\n",
			 nt_errstr(nt_status)));
		prs_mem_free(&rpc_out);
		data_blob_free(&client_reply);
		data_blob_free(&server_response);
                                ...
                                enum pipe_auth_level auth_level)
{
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG(1,("rpc_finish_spnego_ntlmssp_bind: NTLMSSP update using server blob failed.\n"));
		data_blob_free(&client_reply);
		return nt_status;
	}
	...
	if(!smb_io_rpc_hdr("rpc_hdr   ", phdr, rbuf, 0)) {
		DEBUG(1,("rpc_finish_spnego_ntlmssp_bind: Failed to unmarshall RPC_HDR.\n"));
		return NT_STATUS_BUFFER_TOO_SMALL;
	}
		       ...
		       struct cli_pipe_auth_data *auth)
{
	...
	if(!smb_io_rpc_hdr("hdr"   , &hdr, &rbuf, 0)) {
		DEBUG(1,("rpc_pipe_bind: failed to unmarshall RPC_HDR.\n"));
		prs_mem_free(&rbuf);
		return NT_STATUS_BUFFER_TOO_SMALL;
	}

	if(!smb_io_rpc_hdr_ba("", &hdr_ba, &rbuf, 0)) {
		DEBUG(1,("rpc_pipe_bind: Failed to unmarshall RPC_HDR_BA.\n"));
		prs_mem_free(&rbuf);
		return NT_STATUS_BUFFER_TOO_SMALL;
	}
	...
	switch(cli->auth->auth_type) {
		...
		default:
			DEBUG(1,("cli_finish_bind_auth: unknown auth type "
	...
	if (cli->auth->auth_type == PIPE_AUTH_TYPE_NTLMSSP
		...
		if (cli->auth->auth_level == PIPE_AUTH_LEVEL_INTEGRITY) {
			if (!(cli->auth->a_u.ntlmssp_state->neg_flags & NTLMSSP_NEGOTIATE_SIGN)) {
				DEBUG(1,("cli_finish_bind_auth: requested NTLMSSSP signing and server refused.\n"));
				prs_mem_free(&rbuf);
				return NT_STATUS_INVALID_PARAMETER;
			}
		}
		if (cli->auth->auth_level == PIPE_AUTH_LEVEL_INTEGRITY) {
			if (!(cli->auth->a_u.ntlmssp_state->neg_flags & NTLMSSP_NEGOTIATE_SEAL)) {
				DEBUG(1,("cli_finish_bind_auth: requested NTLMSSSP sealing and server refused.\n"));
				prs_mem_free(&rbuf);
				return NT_STATUS_INVALID_PARAMETER;
			}
		}
					    ...
					    struct rpc_pipe_client **presult)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1, ("rpccli_anon_bind_data returned %s\n",
			  nt_errstr(status)));
		TALLOC_FREE(result);
		return status;
	}
						   ...
						   struct rpc_pipe_client **presult)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1, ("rpccli_ntlmssp_bind_data returned %s\n",
			  nt_errstr(status)));
		goto err;
	}
	...
	status = rpc_pipe_bind(result, auth);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1, ("cli_rpc_pipe_open_ntlmssp_internal: cli_rpc_pipe_bind failed with error %s\n",
			nt_errstr(status) ));
		goto err;
	}
						...
						uint32 *pneg_flags)
{
	...
	if (!get_trust_pw_hash(domain, machine_pwd, &machine_account,
			       &sec_chan_type))
	{
		DEBUG(1, ("get_schannel_session_key: could not fetch "
					     ...
					     struct rpc_pipe_client **presult)
{
	...
	status = rpccli_schannel_bind_data(result, domain, auth_level,
					   pdc->sess_key, &auth);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1, ("rpccli_schannel_bind_data returned %s\n",
			  nt_errstr(status)));
		TALLOC_FREE(result);
		return status;
	}
	...
	status = rpc_pipe_bind(result, auth);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1, ("cli_rpc_pipe_open_schannel_with_key: "
						 ...
						 struct rpc_pipe_client **presult)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1,("cli_rpc_pipe_open_ntlmssp_auth_schannel: failed to get schannel session "
				    ...
				    struct rpc_pipe_client **presult)
{
	...
	*presult = NULL;
	...
	status = get_schannel_session_key(cli, domain, &neg_flags,
					  &netlogon_pipe);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1,("cli_rpc_pipe_open_schannel: failed to get schannel session "
			"key from server %s for domain %s.\n",
			cli->desthost, domain ));
		return status;
	}
				...
				struct rpc_pipe_client **presult)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1, ("rpccli_kerberos_bind_data returned %s\n",
			  nt_errstr(status)));
		TALLOC_FREE(result);
		return status;
	}
	...
	status = rpc_pipe_bind(result, auth);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(1, ("cli_rpc_pipe_open_krb5: cli_rpc_pipe_bind failed "
			  "with error %s\n", nt_errstr(status)));
		TALLOC_FREE(result);
		return status;
	}
	...
	*presult = result;
	return NT_STATUS_OK;
#else
	DEBUG(1,("cli_rpc_pipe_open_krb5: kerberos not found at compile time.\n"));
	return NT_STATUS_NOT_IMPLEMENTED;
#endif
}
