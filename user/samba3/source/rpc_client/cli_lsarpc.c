					       ...
					       bool use_lookupsids3)
{
	...
	if (!tmp_ctx) {
		DEBUG(1, ("rpccli_lsa_lookup_sids_noalloc: out of memory!\n"));
	...
	for (i = 0; i < num_sids; i++) {
		...
		if (dom_idx != 0xffffffff) {
			...
			if (name) {
				...
				if ((names)[i] == NULL) {
					DEBUG(1, ("cli_lsa_lookup_sids_noalloc(): out of memory\n"));
					result = NT_STATUS_UNSUCCESSFUL;
					goto done;
				}
			} else {
					       ...
					       bool use_lookupsids3)
{
	...
	if (num_sids) {
		if (!(domains = TALLOC_ARRAY(mem_ctx, char *, num_sids))) {
			DEBUG(1, ("rpccli_lsa_lookup_sids(): out of memory\n"));
						...
						bool use_lookupnames4)
{
	...
	if (num_names) {
		if (!((*sids = TALLOC_ARRAY(mem_ctx, DOM_SID, num_names)))) {
			DEBUG(1, ("cli_lsa_lookup_sids(): out of memory\n"));
			result = NT_STATUS_NO_MEMORY;
			goto done;
		}
