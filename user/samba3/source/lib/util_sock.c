...
uint32 interpret_addr(const char *str)
{
	...
	if (is_ipaddress_v4(str)) {
		...
		if (inet_pton(AF_INET, str, &dest) <= 0) {
			...
			DEBUG(1,("interpret_addr: inet_pton failed "
				"host %s\n",
				str));
			return 0;
		}
...
void set_socket_options(int fd, const char *options)
{
	...
	while (next_token_talloc(ctx, &options, &tok," \t,")) {
		...
		if (!socket_options[i].name) {
			DEBUG(1,("Unknown socket option %s\n",tok));
			continue;
		}
		...
		while (nread < mincnt) {
			...
			if (readret == -1) {
				if (fd == get_client_fd()) {
					...
					DEBUG(1,("read_socket_with_timeout: "
	...
	for (nread=0; nread < mincnt; ) {
		...
		if (selrtn == -1) {
			...
			if (fd == get_client_fd()) {
				 ...
				 * what client failed. */
				DEBUG(1,("read_socket_with_timeout: timeout "
...
ssize_t write_data(int fd, const char *buffer, size_t N)
{
	...
	if (fd == get_client_fd()) {
		 ...
		 */
		DEBUG(1, ("write_data: write failure in writing to client %s. "
			  "Error %s\n", get_peer_addr(fd,addr,sizeof(addr)),
			  strerror(errno)));
	} else {
		...
		socklen_t len)
{
	...
	if (ailist->ai_canonname == NULL ||
		...
		DEBUG(1,("matchname: host name/name mismatch: %s != %s\n",
			 remotehost,
			 ailist->ai_canonname ?
	...
	DEBUG(1,("matchname: host name/address mismatch: %s != %s\n",
		print_sockaddr_len(addr_buf,
			sizeof(addr_buf),
			pss,
			len),
		     ...
		     mode_t dir_perms)
{
	...
	if (sock == -1) {
		DEBUG(1, ("create_pipe_sock: socket error %s\n",
			strerror(errno) ));
                goto out_close;
	}
...
const char *get_mydnsfullname(void)
{
	...
	if (gethostname(my_hostname, sizeof(my_hostname)) == -1) {
		DEBUG(1,("get_mydnsfullname: gethostname failed\n"));
		return NULL;
	}
