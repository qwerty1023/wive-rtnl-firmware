		  ...
		  bool copy_timestamps, bool is_file)
{
	...
	if (copy_acls) {
		...
		if (!sd) {
			DEBUG(1,("failed to get security descriptor: %s\n",
	...
	if (copy_attrs || copy_timestamps) {
		...
		if (!cli_getattrE(cli_share_src, fnum_src, &attr, NULL,
			...
			DEBUG(1,("failed to get file-attrs: %s\n",
				cli_errstr(cli_share_src)));
			nt_status = cli_nt_error(cli_share_src);
			goto out;
		}
	...
	if (fnum_dst == -1) {
		DEBUG(1,("failed to open %s on the destination server: %s: %s\n",
	...
	if (copy_timestamps) {
		...
		if (!cli_setattrE(cli_share_dst, fnum_dst, f_ctime, f_atime, f_mtime)) {
			DEBUG(1,("failed to set file-attrs (timestamps): %s\n",
	...
	if (copy_acls) {
		...
		if (!cli_set_secdesc(cli_share_dst, fnum_dst, sd)) {
			DEBUG(1,("could not set secdesc on %s: %s\n",
	...
	if (copy_attrs) {
		...
		if (!cli_setatr(cli_share_dst, dst_name, attr, 0)) {
			DEBUG(1,("failed to set file-attrs: %s\n",
				cli_errstr(cli_share_dst)));
			nt_status = cli_nt_error(cli_share_dst);
			goto out;
		}
